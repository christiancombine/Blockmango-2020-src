...
function UITipDialog:onLoad()
	...
	self.title = self:getChildWindow("SkyBlockTipDialog-Titile-Name")
	self.title:SetText(Lang:getString("gui_dialog_tip_title_tip"))

	self.cancelBtn = self:getChildWindow("SkyBlockTipDialog-Cancel", GUIType.Button)
	...
	self.sureBtn = self:getChildWindow("SkyBlockTipDialog-Sure", GUIType.Button)
	self.sureBtn:SetText(Lang:getString("gui_dialog_tip_btn_sure"))
    self.sureBtn:registerEvent(GUIEvent.ButtonClick, function()
    	self:onClickSure()
	end)

	self.closeBtn = self:getChildWindow("SkyBlockTipDialog-Close-Btn", GUIType.Button)
    self.closeBtn:registerEvent(GUIEvent.ButtonClick, function()
    	self:onClickClose()
	end)

	self.pricePanel = self:getChildWindow("SkyBlockTipDialog-Price-Panel", GUIType.Layout)
	self.pricePanel:SetVisible(false)

	self.message = self:getChildWindow("SkyBlockTipDialog-Message")
	self.smallMessage = self:getChildWindow("SkyBlockTipDialog-Small-Tips")
...
GUITipDialog = UITipDialog.new("SkyBlockTipDialog.json")
GUITipDialog:hide()