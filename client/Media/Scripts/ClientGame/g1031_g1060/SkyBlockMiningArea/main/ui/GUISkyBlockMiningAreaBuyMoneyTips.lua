...
function UIBuyMoneyTips:onLoad()
	...
	self.stTitle = self:getChildWindow("SkyBlockMiningAreaBuyMoneyTips-Titile-Name")
	self.stTitle:SetText(Lang:getString("gui_dialog_tip_title_tip"))


	self.stMessage = self:getChildWindow("SkyBlockMiningAreaBuyMoneyTips-Message")
	self.stMessage:SetText(Lang:getString("gui_buy_money_tips_left"))

	self.stSmallTips = self:getChildWindow("SkyBlockMiningAreaBuyMoneyTips-Small-Tips")

	self.stSelectText = self:getChildWindow("SkyBlockMiningAreaBuyMoneyTips-Money-Num-Text")
	self.stSelectText:SetText(Lang:getString("gui_select_buy_money_num"))

	self.stMoneyNum = self:getChildWindow("SkyBlockMiningAreaBuyMoneyTips-Select-Money-Num")
	self.moneySlider = self:getChildWindow("SkyBlockMiningAreaBuyMoneyTips-Money-Slider", GUIType.Slider)
	...
	self.reduceBtn = self:getChildWindow("SkyBlockMiningAreaBuyMoneyTips-Reduce-Btn", GUIType.Button)
	self.reduceBtn:registerEvent(GUIEvent.ButtonClick, function()
    	self:onReduceProgress()
	end)

	self.addBtn = self:getChildWindow("SkyBlockMiningAreaBuyMoneyTips-Add-Btn", GUIType.Button)
	self.addBtn:registerEvent(GUIEvent.ButtonClick, function()
    	self:onAddProgress()
	end)

	self.maxBtn = self:getChildWindow("SkyBlockMiningAreaBuyMoneyTips-Max-Btn", GUIType.Button)
	self.maxBtn:registerEvent(GUIEvent.ButtonClick, function()
    	self:onMaxProgress()
	end)

	self.closeBtn = self:getChildWindow("SkyBlockMiningAreaBuyMoneyTips-Close-Btn", GUIType.Button)
    self.closeBtn:registerEvent(GUIEvent.ButtonClick, function()
    	self:onClickClose()
	end)

	self.cancleBtn = self:getChildWindow("SkyBlockMiningAreaBuyMoneyTips-Cancel", GUIType.Button)
	...
	self.sureBtn = self:getChildWindow("SkyBlockMiningAreaBuyMoneyTips-Sure", GUIType.Button)
...
GUIBuyMoneyTips = UIBuyMoneyTips.new("SkyBlockMiningAreaBuyMoneyTips.json")
