require "ui.window.GUIGameBookItem"
...
function UIBook:initRegister()
    ...
    self:registerEvent(GetNewTowerBookEvent, function()
        self:updateItemList()
    end)
end

function UIBook:showBook()
    GameAnalytics:design(1, { "G_book" })
    self:updateItemList()
    self:show()
end
...
function UIBook:updateItemList()
    ...
    for _, config in pairs(bookConfig) do
        if towerList ~= nil and towerList[config.Id] ~= nil then
