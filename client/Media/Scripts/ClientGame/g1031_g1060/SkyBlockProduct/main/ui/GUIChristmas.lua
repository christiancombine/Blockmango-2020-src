...
function UIChristmas:onLoadInitUI()
    ...
    self.buyGiftTab = self:getChildWindow("SkyBlockProductChristmas-Buy-Gift-Tab", GUIType.RadioButton)
    ...
    self.openGiftTab = self:getChildWindow("SkyBlockProductChristmas-Open-Gift-Tab", GUIType.RadioButton)
    ...
    self.closeBtn = self:getChildWindow("SkyBlockProductChristmas-BtnClose", GUIType.Button)
    self.closeBtn:registerEvent(GUIEvent.ButtonClick, function()
        self:onClosePanel()
    end)

    self.llBuyGift = self:getChildWindow("SkyBlockProductChristmas-Buy-Gift-Panel", GUIType.Layout)
    self.llBuyGift:SetVisible(false)
    self.llOpenGift = self:getChildWindow("SkyBlockProductChristmas-Open-Gift-Panel", GUIType.Layout)
    self.llOpenGift:SetVisible(false)

    self.buySmallImg = self:getChildWindow("SkyBlockProductChristmas-Buy-Gift-Small", GUIType.StaticImage)
    self.openSmallImg = self:getChildWindow("SkyBlockProductChristmas-Open-Gift-Small", GUIType.StaticImage)
    self.buySmallImg:SetVisible(false)
    self.openSmallImg:SetVisible(false)

    self.llBuyGiftLaout = self:getChildWindow("SkyBlockProductChristmas-Buy-Gift-List", GUIType.Layout)
    self.buyItemGridView = IGUIGridView.new("SkyBlockProductChristmas-Buy-Item-grid-view", self.llBuyGiftLaout)
    self.buyItemGridView:setConfig(25, 0, 2)
    self.buyItemGridView.root:SetMoveAble(false)


    self.openSmallIcon = self:getChildWindow("SkyBlockProductChristmas-Gift-Small-Icon", GUIType.StaticImage)
    self.openBigImg = self:getChildWindow("SkyBlockProductChristmas-Big-Icon", GUIType.StaticImage)
    
    self.toLeftBtn = self:getChildWindow("SkyBlockProductChristmas-To-Left-Btn", GUIType.Button)
    self.toLeftBtn:registerEvent(GUIEvent.ButtonClick, function()
        self:toLeftBtnEvent()
    end)    

    self.toRightBtn = self:getChildWindow("SkyBlockProductChristmas-To-Right-Btn", GUIType.Button)
    self.toRightBtn:registerEvent(GUIEvent.ButtonClick, function()
        self:toRightBtnEvent()
    end)
    self.stGiftName = self:getChildWindow("SkyBlockProductChristmas-Gift-Name")

    self.llOpenLeft = self:getChildWindow("SkyBlockProductChristmas-Left-Open-Panel", GUIType.Layout)
    self.llOpenLeft:SetVisible(false)
    self.openLeftBtn = self:getChildWindow("SkyBlockProductChristmas-Left-Open-Btn", GUIType.Button)
    self.openLeftBtn:registerEvent(GUIEvent.ButtonClick, function()
        self:openLeftBtnEvent()
    end)
    self.stLeftOpenText = self:getChildWindow("SkyBlockProductChristmas-Left-Price-Num")
    self.leftOpenPriceIcon = self:getChildWindow("SkyBlockProductChristmas-Left-Price-Icon", GUIType.StaticImage)

    self.llBuyGiftBg = self:getChildWindow("SkyBlockProductChristmas-Buy-Gift-Bg", GUIType.StaticImage)
    self.llOpenGiftBg = self:getChildWindow("SkyBlockProductChristmas-Open-Gift-Bg", GUIType.StaticImage)
    self.llBuyGiftBg:SetVisible(false)
    self.llOpenGiftBg:SetVisible(false)

    
    self.llOpenRight = self:getChildWindow("SkyBlockProductChristmas-Right-Open-Panel", GUIType.Layout)
    self.llOpenRight:SetVisible(false)
    self.openRightBtn = self:getChildWindow("SkyBlockProductChristmas-Right-Open-Btn", GUIType.Button)
    self.openRightBtn:registerEvent(GUIEvent.ButtonClick, function()
        self:openRightBtnEvent()
    end)
    self.stRightOpenText = self:getChildWindow("SkyBlockProductChristmas-Right-Price-Num")
    self.rightOpenPriceIcon = self:getChildWindow("SkyBlockProductChristmas-Right-Price-Icon", GUIType.StaticImage)
    
    self.stMayHaveText = self:getChildWindow("SkyBlockProductChristmas-May-Have-Text")
    self.stMayHaveText:SetText(Lang:getString("gui_open-may-have"))
    self.llMayHave = self:getChildWindow("SkyBlockProductChristmas-Reward-List", GUIType.Layout)
    self.mayHaveGridView = IGUIGridView.new("SkyBlockProductChristmas-open-may-have-grid-view", self.llMayHave)
    self.mayHaveGridView:setConfig(5, 5, 5)

    self.llRewardHide = self:getChildWindow("SkyBlockProductChristmas-Reward-Hide-Layout")
    self.llRewardHide:SetVisible(true)

    self.stGiftNum = self:getChildWindow("SkyBlockProductChristmas-My-Have-Num")

    self.llRewardResult = self:getChildWindow("SkyBlockProductChristmas-Reward-Result-Panel", GUIType.Layout)
    self.resultbigBgImg = self:getChildWindow("SkyBlockProductChristmas-Result-Big-Bg", GUIType.StaticImage)
    self.resultbigBgImg:registerEvent(GUIEvent.TouchUp, function()
        self.llRewardResult:SetVisible(false)
        self.llRewardHide:SetVisible(true)
    end)

    self.resultbigBgImg = self:getChildWindow("SkyBlockProductChristmas-Result-Ok-Btn", GUIType.Button)
    ...
    self.llRewardIcon = self:getChildWindow("SkyBlockProductChristmas-Result-Icon", GUIType.Layout)
    self.rewardIconGridView = IGUIGridView.new("SkyBlockProductChristmas-open-reward_icon-grid-view", self.llRewardIcon)
    self.rewardIconGridView:setConfig(0, 0, 1)
    self.rewardIconItem = UISkyBlockItemIcon.new("SkyBlockProductItemIcon.json")
    ...
    self.stLastTime = self:getChildWindow("SkyBlockProductChristmas-Last-Time")
    self.stLastTimeText = self:getChildWindow("SkyBlockProductChristmas-Last-Time-Text")
    self.stLastTimeText:SetText(Lang:getString("gui_gift_last_time"))
    self.stInvHaveText = self:getChildWindow("SkyBlockProductChristmas-My-Have-Num-Text")
    self.stInvHaveText:SetText(Lang:getString("gui_inv_have_gift_num"))
    self.stRewardText = self:getChildWindow("SkyBlockProductChristmas-Reward-Text")
    self.stRewardText:SetText(Lang:getString("gui_sky_block_pri_reward_text"))

    self.btnProbability = self:getChildWindow("SkyBlockProductChristmas-Probability", GUIType.Button)
...
function UIChristmas:upDatePanel()
    ...
    for i, v in pairs(cfg) do
        local curItem = UIChirstmasGiftItem.new("SkyBlockProductChristmasBuyItem.json")
...
function UIChristmas:upDateMayHavePanel()
    ...
    for i, v in pairs(cfg.Reward) do
        ...
        if cue_item_cfg then
            local curItem = UISkyBlockItemIcon.new("SkyBlockProductItemIcon.json")
...
function UIChristmas:showPanelByIndex(index)
    ...
    GUISkyBlockProductMoneyDeatil:show()
end
...
GUIChristmas = UIChristmas.new("SkyBlockProductChristmas.json")
GUIChristmas:hide()