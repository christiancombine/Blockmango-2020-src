...
function Area:BlockPlaceArea(vec3)
    return vec3.x >= tonumber(self.minX) and vec3.x < tonumber(self.maxX)
            and vec3.y >= tonumber(self.minY) and vec3.y < tonumber(self.maxY)
            and vec3.z >= tonumber(self.minZ) and vec3.z < tonumber(self.maxZ)
end

function Area:compareMinArea(a, b)
    if a > b then
        return b
    else
        return a
...
function Area:compareMaxArea(a, b)
    if a > b then
        return a
    else
        return b
