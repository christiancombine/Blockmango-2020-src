...
function TowerConfig:init(config)
    for _, vConfig in pairs(config) do
        ...
        data.killAdditionIds = StringUtil.split(vConfig.killAdditionIds or "", "#")
        data.levelGroup = tonumber(vConfig.levelGroup or "0")
        data.AttackSound = StringUtil.split(vConfig.AttackSound or "0", "#")
        data.AppearSound = tonumber(vConfig.AppearSound or "0")
        data.SellSound = tonumber(vConfig.SellSound or "0")
        data.isShortCutPlace = tonumber(vConfig.isShortCutPlace or 0)
        data.bulletCountPerSecond = data.scatterCount / data.AttackSpeed * 1000
        Config[data.Id] = data
...
function TowerConfig:getTowerListByStar(starLevel)
    return StarList[tostring(starLevel)]
end

function TowerConfig:SortStarList()
    for _, list in pairs(StarList) do
        table.sort(list, function(info1, info2)
            if info1.miss ~= info2.miss then
                return info1.Id < info2.Id
            end
            return tonumber(info1.ShowSeq) < tonumber(info2.ShowSeq)
        end)
    end
end
