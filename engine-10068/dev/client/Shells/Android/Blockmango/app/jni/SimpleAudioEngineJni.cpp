...
extern "C"
{
    ...
    void playBackgroundMusicJNI(const char *path, bool isLoop, float fMinInterval, float fMaxInterval)
    {
        // void playBackgroundMusic(String,boolean)
        
        JniMethodInfo methodInfo;
        
        if (! getStaticMethodInfo(methodInfo, "playBackgroundMusic", "(Ljava/lang/String;ZFF)V"))
        {
            return;
        }
        
        jstring stringArg = methodInfo.env->NewStringUTF(path);
        methodInfo.env->CallStaticVoidMethod(methodInfo.classID, methodInfo.methodID, stringArg, isLoop, fMinInterval, fMaxInterval);
        methodInfo.env->DeleteLocalRef(stringArg);
        methodInfo.env->DeleteLocalRef(methodInfo.classID);
    }
    ...
    unsigned int playEffectJNI(const char* path, bool bLoop, float fMinInterval, float fMaxInterval)
    {
