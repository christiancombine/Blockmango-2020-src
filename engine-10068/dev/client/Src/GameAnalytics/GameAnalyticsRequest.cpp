...
namespace BLOCKMAN
{
	std::string GameAnalyticsRequest::getAuthorization(const std::string& src)
	{
		...
		HMAC<SHA256>::calc(src, GameAnalyticsSetting::Instance()->SECRET_KEY.c_str(), digest);
	...
	void GameAnalyticsRequest::asyncInit(int retryTimes)
	{
		...
		auto callback = [=](cpr::Response response)
		{
			...
			else
			{
				...
				auto foo = [=]()
				{
					SCRIPT_EVENT::GameAnalyticsInitEvent::invoke(Enabled, ServerTs, GameAnalyticsSetting::Instance()->CUSTOM_01, 
						GameAnalyticsSetting::Instance()->CUSTOM_02);
				};
				GameClient::CGame::Instance()->addAsyncCall(foo);
			}
		};
		...
		CprCallBackWrap::Instance()->PostCallback(callback,
			...
			cpr::Url{ GameAnalyticsSetting::Instance()->INIT_URL.c_str() },
			cpr::Parameters{ },
			cpr::Body{ body },
			header
		);
	...
	void GameAnalyticsRequest::asyncEvents(String body, int retryTimes)
	{
		asyncPost(GameAnalyticsSetting::Instance()->EVENT_URL, GameAnalyticsSetting::Instance()->CLIENT_IP, body, false, retryTimes);
	}
