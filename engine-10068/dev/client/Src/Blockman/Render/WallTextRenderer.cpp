...
namespace BLOCKMAN
{
	void WallTextRenderer::renderWallText(const LORD::String& txt, LORD::Vector3 pos, bool canBeOccluded, float scale, float yaw, float pitch, float r, float g, float b, float a, WallTextAlignment alignment)
	{
		struct HeadTextInfo
		{
			GUIString text;
		...
		for (auto& txt_line : txt_lines)
		{
			...
			float offsetKey = 0.5f;
			if (alignment == WallTextAlignment::Left)
				offsetKey = 0;
			if (alignment == WallTextAlignment::Right)
				offsetKey = 1;

			for (auto& it : buffer_line)
				it.pos.x -= totalWidth * offsetKey;
