...
namespace BLOCKMAN
{
	...
	void GuiBedWarRuneItem::removeComponents()
	{
		if (m_window)
		{
	...
	vector<GUIWindow*>::type GuiBedWarRuneItem::getPrivateChildren()
	{
		return { m_window };
	}

	GuiBedWarRuneItem::GuiBedWarRuneItem(const GUIWindowType & nType, const GUIString & strName)
		: BaseClass(nType, strName)
	{
		m_window = GUIWindowManager::Instance()->LoadWindowFromJSON("BedWarRuneItem.json");
		...
		m_rune_btn->subscribeEvent(EventButtonClick, std::bind(&GuiBedWarRuneItem::onRuneClick, this, std::placeholders::_1));
	...
	void GuiBedWarRuneItem::UpdateSelf(ui32 nTimeElapse)
	{
	...
	void GuiBedWarRuneItem::Destroy()
	{
		removeComponents();
		BaseClass::Destroy();
	}

	void GuiBedWarRuneItem::RefreshItem(int id, int num)
	{
		m_rune_id = id;
		m_rune_num = num;
	...
	bool GuiBedWarRuneItem::onRuneClick(const EventArgs & events)
	{
		if (click_cd <= 0)
		{
			BedWarRuneClickEvent::emit(m_rune_id);
			click_cd = CLICK_CD;
		}
		else
		{
			LordLogInfo("GuiBedWarRuneItem::onRuneClick cd");
		}
		return true;
	}
}