#include "gui_blockCityStoreBlock.h"
...
namespace BLOCKMAN
{
	gui_blockCityStoreBlock::gui_blockCityStoreBlock()
		: gui_layout("BlockCityStoreBlock.json")
	{
	}
	...
	void gui_blockCityStoreBlock::onLoad()
	{
		m_tabs = getWindow<GUILayout>("BlockCityStoreBlock-Tabs");
		m_itemsView = getWindow<GUIGridView>("BlockCityStoreBlock-Items");
		m_itemsView->InitConfig(3.f, 3.f, 5);
		m_subscriptionGuard.add(UpdateBlockCityStoreEvent::subscribe(std::bind(&gui_blockCityStoreBlock::onDataChange, this)));
		m_subscriptionGuard.add(BlockCityStoreItemClickEvent::subscribe(std::bind(&gui_blockCityStoreBlock::onItemClick, this, std::placeholders::_1)));
	}

	void gui_blockCityStoreBlock::onShow()
	{
		if (m_dataChange)
		{
			initData();
	...
	void gui_blockCityStoreBlock::onUpdate(ui32 nTimeElapse)
	{

	}

	bool gui_blockCityStoreBlock::onDataChange()
	{
		m_dataChange = true;
		if (isShown())
		{
	...
	bool gui_blockCityStoreBlock::isNeedLoad()
	{
		return checkGameType({ ClientGameType::BlockCity ,ClientGameType::BlockFort });
	}

	void gui_blockCityStoreBlock::initData()
	{
	...
	void gui_blockCityStoreBlock::initTabs(std::vector<BlockCityStoreGroup>& groups)
	{
		m_radioMap.clear();
		static int count = 0;
		if (groups.size() != m_tabs->GetChildCount())
		{
			...
			{
				GUIString tabName = StringUtil::Format("BlockCityStoreBlock-Tabs-Item-%d", count++).c_str();
				...
				radioItem->subscribeEvent(EventRadioStateChanged, std::bind(&gui_blockCityStoreBlock::onRadioChange, this, std::placeholders::_1, groups[i].groupId));
				radioItem->SetText(getString(groups[i].groupName.c_str()));
				radioItem->SetTextColor(Color::BLACK);

				GUIString tabIconName = StringUtil::Format("BlockCityStoreBlock-Tabs-Icon-Item-%d", count).c_str();
				...
				GUIString tabSignName = StringUtil::Format("BlockCityStoreBlock-Tabs-Sign-%d", count).c_str();
	...
	void gui_blockCityStoreBlock::updateItems(std::vector<BlockCityStoreItem>& items)
	{
		if (!UICommon::checkBlockCityParam())
		{
			return;
		...
		if (items.size() != m_itemsView->GetItemCount())
		{
			...
			{
				String iterLayout = StringUtil::Format("BlockCityStoreBlock-Items-%d", count++).c_str();
	...
	bool gui_blockCityStoreBlock::onRadioChange(const EventArgs & args, i32 groupId)
	{
	...
	bool gui_blockCityStoreBlock::onItemClick(const BlockCityStoreItem & item)
	{
		if (!isShown())
		{
			return false;
	...
	UVector2 gui_blockCityStoreBlock::getItemPosition()
	{
		for (size_t i = 0; i < m_itemsView->GetItemCount(); i++)
		{
			auto itemView = dynamic_cast<GuiBlockCityTigerLotteryItem*>(m_itemsView->GetItem(i));
