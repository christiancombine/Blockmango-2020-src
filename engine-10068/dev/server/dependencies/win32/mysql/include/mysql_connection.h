...
private:
	...
	Driver * driver;
	std::shared_ptr< NativeAPI::NativeConnectionWrapper > proxy; // replace boost::shared_ptr by std::shared_ptr, by oujie

	/* statement handle to execute queries initiated by driver. Perhaps it is
	   a good idea to move it to a separate helper class */
	std::auto_ptr< ::sql::mysql::MySQL_Statement > service; //replace boost::scoped_ptr by std::auto_ptr, by oujie

	std::auto_ptr< ::sql::mysql::MySQL_ConnectionData > intern; /* pimpl */ //replace boost::scoped_ptr by std::auto_ptr, by oujie
