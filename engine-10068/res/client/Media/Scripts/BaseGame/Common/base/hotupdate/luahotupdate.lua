...
function HU.DebugNotify(...)
    if HU.DebugNotifyFunc then
        HU.DebugNotifyFunc(...)
    end
end
...
function HU.ResetENV(object, name, From, Deepth)
    ...
    local function f(object, name)
        ...
        if type(object) == "function" then
            HU.DebugNotify(Deepth .. "HU.ResetENV", name, "  from:" .. From)
            xpcall(function()
...
function HU.UpdateUpvalue(OldFunction, NewFunction, Name, From, Deepth)
    HU.DebugNotify(Deepth .. "HU.UpdateUpvalue", Name, "  from:" .. From)
    local OldUpvalueMap = {}
    local OldExistName = {}
    for i = 1, math.huge do
...
function HU.UpdateOneFunction(OldObject, NewObject, FuncName, OldTable, From, Deepth)
    ...
    HU.DebugNotify(Deepth .. "HU.UpdateOneFunction " .. FuncName .. "  from:" .. From)
...
function HU.UpdateAllFunction(OldTable, NewTable, Name, From, Deepth)
    ...
    HU.VisitedSig[signature] = true
    HU.DebugNotify(Deepth .. "HU.UpdateAllFunction " .. Name .. "  from:" .. From)
