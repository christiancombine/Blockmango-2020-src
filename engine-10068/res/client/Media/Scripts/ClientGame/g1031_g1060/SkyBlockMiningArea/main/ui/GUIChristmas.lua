...
function UIChristmas:onLoadInitUI()
    ...
    self.buyGiftTab = self:getChildWindow("SkyBlockMiningAreaChristmas-Buy-Gift-Tab", GUIType.RadioButton)
    ...
    self.openGiftTab = self:getChildWindow("SkyBlockMiningAreaChristmas-Open-Gift-Tab", GUIType.RadioButton)
    ...
    self.closeBtn = self:getChildWindow("SkyBlockMiningAreaChristmas-BtnClose", GUIType.Button)
    self.closeBtn:registerEvent(GUIEvent.ButtonClick, function()
        self:onClosePanel()
    end)

    self.llBuyGift = self:getChildWindow("SkyBlockMiningAreaChristmas-Buy-Gift-Panel", GUIType.Layout)
    self.llBuyGift:SetVisible(false)
    self.llOpenGift = self:getChildWindow("SkyBlockMiningAreaChristmas-Open-Gift-Panel", GUIType.Layout)
    self.llOpenGift:SetVisible(false)

    self.buySmallImg = self:getChildWindow("SkyBlockMiningAreaChristmas-Buy-Gift-Small", GUIType.StaticImage)
    self.openSmallImg = self:getChildWindow("SkyBlockMiningAreaChristmas-Open-Gift-Small", GUIType.StaticImage)
    self.buySmallImg:SetVisible(false)
    self.openSmallImg:SetVisible(false)

    self.llBuyGiftLaout = self:getChildWindow("SkyBlockMiningAreaChristmas-Buy-Gift-List", GUIType.Layout)
    self.buyItemGridView = IGUIGridView.new("SkyBlockMiningAreaChristmas-Buy-Item-grid-view", self.llBuyGiftLaout)
    self.buyItemGridView:setConfig(25, 0, 2)
    self.buyItemGridView.root:SetMoveAble(false)


    self.openSmallIcon = self:getChildWindow("SkyBlockMiningAreaChristmas-Gift-Small-Icon", GUIType.StaticImage)
    self.openBigImg = self:getChildWindow("SkyBlockMiningAreaChristmas-Big-Icon", GUIType.StaticImage)
    
    self.toLeftBtn = self:getChildWindow("SkyBlockMiningAreaChristmas-To-Left-Btn", GUIType.Button)
    self.toLeftBtn:registerEvent(GUIEvent.ButtonClick, function()
        self:toLeftBtnEvent()
    end)    

    self.toRightBtn = self:getChildWindow("SkyBlockMiningAreaChristmas-To-Right-Btn", GUIType.Button)
    self.toRightBtn:registerEvent(GUIEvent.ButtonClick, function()
        self:toRightBtnEvent()
    end)
    self.stGiftName = self:getChildWindow("SkyBlockMiningAreaChristmas-Gift-Name")

    self.llOpenLeft = self:getChildWindow("SkyBlockMiningAreaChristmas-Left-Open-Panel", GUIType.Layout)
    self.llOpenLeft:SetVisible(false)
    self.openLeftBtn = self:getChildWindow("SkyBlockMiningAreaChristmas-Left-Open-Btn", GUIType.Button)
    self.openLeftBtn:registerEvent(GUIEvent.ButtonClick, function()
        self:openLeftBtnEvent()
    end)
    self.stLeftOpenText = self:getChildWindow("SkyBlockMiningAreaChristmas-Left-Price-Num")
    self.leftOpenPriceIcon = self:getChildWindow("SkyBlockMiningAreaChristmas-Left-Price-Icon", GUIType.StaticImage)

    self.llBuyGiftBg = self:getChildWindow("SkyBlockMiningAreaChristmas-Buy-Gift-Bg", GUIType.StaticImage)
    self.llOpenGiftBg = self:getChildWindow("SkyBlockMiningAreaChristmas-Open-Gift-Bg", GUIType.StaticImage)
    self.llBuyGiftBg:SetVisible(false)
    self.llOpenGiftBg:SetVisible(false)

    
    self.llOpenRight = self:getChildWindow("SkyBlockMiningAreaChristmas-Right-Open-Panel", GUIType.Layout)
    self.llOpenRight:SetVisible(false)
    self.openRightBtn = self:getChildWindow("SkyBlockMiningAreaChristmas-Right-Open-Btn", GUIType.Button)
    self.openRightBtn:registerEvent(GUIEvent.ButtonClick, function()
        self:openRightBtnEvent()
    end)
    self.stRightOpenText = self:getChildWindow("SkyBlockMiningAreaChristmas-Right-Price-Num")
    self.rightOpenPriceIcon = self:getChildWindow("SkyBlockMiningAreaChristmas-Right-Price-Icon", GUIType.StaticImage)
    
    self.stMayHaveText = self:getChildWindow("SkyBlockMiningAreaChristmas-May-Have-Text")
    self.stMayHaveText:SetText(Lang:getString("gui_open-may-have"))
    self.llMayHave = self:getChildWindow("SkyBlockMiningAreaChristmas-Reward-List", GUIType.Layout)
    self.mayHaveGridView = IGUIGridView.new("SkyBlockMiningAreaChristmas-open-may-have-grid-view", self.llMayHave)
    self.mayHaveGridView:setConfig(5, 5, 5)

    self.llRewardHide = self:getChildWindow("SkyBlockMiningAreaChristmas-Reward-Hide-Layout")
    self.llRewardHide:SetVisible(true)

    self.stGiftNum = self:getChildWindow("SkyBlockMiningAreaChristmas-My-Have-Num")

    self.llRewardResult = self:getChildWindow("SkyBlockMiningAreaChristmas-Reward-Result-Panel", GUIType.Layout)
    self.resultbigBgImg = self:getChildWindow("SkyBlockMiningAreaChristmas-Result-Big-Bg", GUIType.StaticImage)
    self.resultbigBgImg:registerEvent(GUIEvent.TouchUp, function()
        self.llRewardResult:SetVisible(false)
        self.llRewardHide:SetVisible(true)
    end)

    self.resultbigBgImg = self:getChildWindow("SkyBlockMiningAreaChristmas-Result-Ok-Btn", GUIType.Button)
    ...
    self.llRewardIcon = self:getChildWindow("SkyBlockMiningAreaChristmas-Result-Icon", GUIType.Layout)
    self.rewardIconGridView = IGUIGridView.new("SkyBlockMiningAreaChristmas-open-reward_icon-grid-view", self.llRewardIcon)
    self.rewardIconGridView:setConfig(0, 0, 1)
    self.rewardIconItem = UISkyBlockItemIcon.new("SkyBlockMiningAreaItemIcon.json")
    ...
    self.stLastTime = self:getChildWindow("SkyBlockMiningAreaChristmas-Last-Time")
    self.stLastTimeText = self:getChildWindow("SkyBlockMiningAreaChristmas-Last-Time-Text")
    self.stLastTimeText:SetText(Lang:getString("gui_gift_last_time"))
    self.stInvHaveText = self:getChildWindow("SkyBlockMiningAreaChristmas-My-Have-Num-Text")
    self.stInvHaveText:SetText(Lang:getString("gui_inv_have_gift_num"))
    self.stRewardText = self:getChildWindow("SkyBlockMiningAreaChristmas-Reward-Text")
    self.stRewardText:SetText(Lang:getString("gui_sky_block_pri_reward_text"))

    self.btnProbability = self:getChildWindow("SkyBlockMiningAreaChristmas-Probability", GUIType.Button)
...
function UIChristmas:upDatePanel()
    ...
    for i, v in pairs(cfg) do
        local curItem = UIChirstmasGiftItem.new("SkyBlockMiningAreaChristmasBuyItem.json")
...
function UIChristmas:checkCanOpenGift()
    ...
    if not inv:isCanAddItem(2802, 0, 1) then
...
function UIChristmas:upDateMayHavePanel()
    ...
    for i, v in pairs(cfg.Reward) do
        ...
        if cue_item_cfg then
            local curItem = UISkyBlockItemIcon.new("SkyBlockMiningAreaItemIcon.json")
...
function UIChristmas:showPanelByIndex(index)
    ...
    GUISkyBlockMiningAreaMoneyDeatil:show()
end
...
GUIChristmas = UIChristmas.new("SkyBlockMiningAreaChristmas.json")
GUIChristmas:hide()