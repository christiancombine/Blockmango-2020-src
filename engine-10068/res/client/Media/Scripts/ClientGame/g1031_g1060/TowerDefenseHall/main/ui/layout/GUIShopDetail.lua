...
function UIShopDetail:initRegister()
    ...
    self:registerEvent(GoRechargeEvent, function()
        GameAnalytics:design(1, { "H_buy_gcube" })
    end)
end
...
function UIShopDetail:showDetailPage(itemId)
    self.itemId = itemId
    self:updateDetailInfo(itemId)
    self:show()

    GameAnalytics:design(1, { "H_shop_ID", self.itemId })
end

function UIShopDetail:updateDetailInfo(itemId)
...
function UIShopDetail:onClickBuyButton()
    ...
    if itemConfig.moneyType == Define.Money.DiamondsGold then
        PayHelper.checkMoney(MoneyType.Diamonds, itemConfig.price, function(isEnough)
            if isEnough then
                self:onMoneyEnough(self.itemId)
            end
        end)
    end
    if itemConfig.moneyType == Define.Money.HallGold then
        if GamePlayer:Instance().Wallet:getMoneyByCoinId(Define.Money.HallGold) >= itemConfig.price then
            self:onMoneyEnough(self.itemId)
        else
            self:onMoneyNotEnough(self.itemId, itemConfig.price - GamePlayer:Instance().Wallet:getMoneyByCoinId(Define.Money.HallGold))
        end
    end
end

function UIShopDetail:onMoneyEnough(itemId)
    GamePacketSender:sendBuyShopItem(itemId)
    GameAnalytics:design(1, { "H_shop_ID_buy", itemId })
end

function UIShopDetail:onMoneyNotEnough(itemId, needMoneyNum)
    local DiamondsNum = math.ceil(needMoneyNum / GamePlayer:Instance().ExchangeRate)
    self.isNotCloseTip = false
    GameAnalytics:design(0, { "H_shop_ID_buy", itemId })
