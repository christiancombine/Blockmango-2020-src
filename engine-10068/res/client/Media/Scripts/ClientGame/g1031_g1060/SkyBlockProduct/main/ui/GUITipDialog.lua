...
function UITipDialog:onLoad()
	...
	self.title = self:getChildWindow("SkyBlockProductTipDialog-Titile-Name")
	self.title:SetText(Lang:getString("gui_dialog_tip_title_tip"))

	self.cancelBtn = self:getChildWindow("SkyBlockProductTipDialog-Cancel", GUIType.Button)
	...
	self.sureBtn = self:getChildWindow("SkyBlockProductTipDialog-Sure", GUIType.Button)
	self.sureBtn:SetText(Lang:getString("gui_dialog_tip_btn_sure"))
    self.sureBtn:registerEvent(GUIEvent.ButtonClick, function()
    	self:onClickSure()
	end)

	self.closeBtn = self:getChildWindow("SkyBlockProductTipDialog-Close-Btn", GUIType.Button)
    self.closeBtn:registerEvent(GUIEvent.ButtonClick, function()
    	self:onClickClose()
	end)

	self.pricePanel = self:getChildWindow("SkyBlockProductTipDialog-Price-Panel", GUIType.Layout)
	self.pricePanel:SetVisible(false)

	self.message = self:getChildWindow("SkyBlockProductTipDialog-Message")
	self.smallMessage = self:getChildWindow("SkyBlockProductTipDialog-Small-Tips")
...
GUITipDialog = UITipDialog.new("SkyBlockProductTipDialog.json")
GUITipDialog:hide()