...
#	define USE_FINE_GRAIN_LOCKS
    ...
    static MallocInterface* G_MB = NULL;
        ...
        virtual void* Malloc( size_t Size, unsigned int Alignment )
        {
           ...
           // FlushPendingFrees();
            ...
            if( Size < BinnedSizeLimit )
            {
                ...
                if( !Pool )
                {
                    Pool = AllocatePoolMemory(Table, BINNED_ALLOC_POOL_SIZE/*PageSize*/, (unsigned short)Size);
                }
    ...
    void CreateBinnedMalloc()
    {
        ...
        // get free memory
        vm_size_t PageSize;
        //this page size in ios is error,then use getpagesize() modify by wangyue
...
#elif LORD_PLATFORM == LORD_PLATFORM_WINDOWS
        
        PERFORMANCE_INFORMATION PerformanceInformation = {0};
        ::GetPerformanceInfo( &PerformanceInformation, sizeof(PerformanceInformation) );
        
        
        G_MB = new MallocBinned((unsigned int)(PerformanceInformation.PageSize&0xffffffff), (unsigned long long )(0xffffffff)+1);
...
//	    G_MB = new MallocBinned(PageSize, MemoryLimit);
#else
        G_MB = new MallocBinned(4096, 0x100000000);
    ...
    void* MallocBinnedMgr::Malloc(size_t count, unsigned int aligment)
    {
        ...
        return G_MB->Malloc(count, aligment);
    }
    void* MallocBinnedMgr::Realloc( void* Ptr, size_t NewSize, unsigned int Alignment )
    {
        ...
        return G_MB->Realloc(Ptr, NewSize, Alignment);
    }
	...
	void* BinnedAllocPolicy::allocateBytes(size_t count, const char* file, int line, const char* func)
	{
		void* ptr = MallocBinnedMgr::Malloc(count);
	...
	void BinnedAllocPolicy::deallocateBytes(void* ptr)
	{
#ifdef LORD_MEMORY_TRACKER
...
//	void* BinnedAlignedAllocPolicy<Alignment>::allocateBytes(size_t count, const char* file = 0, int line = 0, const char* func = 0)
//	{
//		void* ptr = MallocBinnedMgr::Malloc(count, Alignment);
//#ifdef LORD_MEMORY_TRACKER
//		MemTracker::get()._recordAlloc(ptr, count, 0, file, line, func);
//#endif
//		return ptr;
...
//	template <size_t Alignment>
//	void BinnedAlignedAllocPolicy<Alignment>::deallocateBytes(void* ptr)
//	{
//#ifdef LORD_MEMORY_TRACKER
//		MemTracker::get()._recordDealloc(ptr);
//#endif
