...
namespace LORD
{
	...
	void* NedPoolingImpl::allocBytesAligned(size_t align, size_t count, const char* file, int line, const char* func)
	{
	...
	void* NedPoolingImplNoMemTrace::allocBytes(size_t count)
	{
		return _NedPoolingIntern::internalAlloc(count);
	}

	void NedPoolingImplNoMemTrace::deallocBytes(void* ptr)
	{
		// deal with null
		if (!ptr)
			return;
	...
	void* NedPoolingImplNoMemTrace::allocBytesAligned(size_t align, size_t count)
	{
		// default to platform SIMD alignment if none specified
		return (align? _NedPoolingIntern::internalAllocAligned(align, count) : _NedPoolingIntern::internalAllocAligned(LORD_SIMD_ALIGNMENT, count));
	}

	void NedPoolingImplNoMemTrace::deallocBytesAligned(size_t align, void* ptr)
	{
		// deal with null
		if (!ptr)
			return;
