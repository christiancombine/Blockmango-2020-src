...
#define ADD_MATH_OP_COUNT
		...
		*/
		...
		static FORCEINLINE bool floatEqual(float A, float B, float epsilon = LOWEPSILON)
		{
			if (A == B)
				return true;
		...
		inline bool checkDistanceLessThan(float x1, float y1, float z1,
			...
			float maxDistance)
		{
			...
			// Compare squared distance to avoid unnecessary sqrt (faster)
