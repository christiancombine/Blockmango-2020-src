...
namespace LORD
{
	...
	*/
	...
	void Quaternion::SlerpWithAngle(Quaternion& quan, const Quaternion& q1, const Quaternion& q2, Real angle)
	{
#if LORD_MODE == LORD_MODE_DEBUG
		++Quaternion::OP_COUNT;
#endif
		...
		fCos = Math::Clamp(fCos, -1.f, 1.f);
		Real fSin = Math::Sqrt(1 - Math::Sqr(fCos));
		Real radian = Math::ATan2(fSin, fCos);
		LordAssert(radian >= 0 && angle >= 0);
		if (angle >= radian)
		{
			quan = q2;
			return;
		...
		LordAssert(fCos >= 0);
		if (fCos < 1 - QUAN_EPSILON)
		{
			Real fInvSin = 1.0f / fSin;
			Real fCoeff0 = Math::Sin(radian - angle) * fInvSin;
			Real fCoeff1 = Math::Sin(angle) * fInvSin;
			quan = fCoeff0 * q1 + fCoeff1 * q;
		}
		else
		{
