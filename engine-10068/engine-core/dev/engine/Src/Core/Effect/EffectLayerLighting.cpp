...
namespace LORD
{
	...
	void EffectLayerLighting::prepare()
	{
		...
		{
			LordSafeFree(mpRandomData[i]);
			mpRandomData[i] = (float*)LordMalloc(sizeof(float)*mSegment*mLightNum); 
		}
		...
		if (mSplineAffected)
		{
			...
			mSplineIntDir = (Vector3*)LordMalloc(sizeof(Vector3)*(mSegment+1));
			mSplineIntUp = (Vector3*)LordMalloc(sizeof(Vector3)*(mSegment+1));
			mSplineRandPos = (Vector3*)LordMalloc(sizeof(Vector3)*(mSegment));
	...
	void EffectLayerLighting::unprepare()
	{
		...
		LordSafeFree(mSplineIntDir);
		LordSafeFree(mSplineIntUp);
		LordSafeFree(mSplineRandPos);
	...
	void EffectLayerLighting::buildPath(const Matrix4& mat)
	{
		...
		else
		{
			...
			if (vDir.lenSqr() < 0.00001)
			{
				vDir = Vector3::UNIT_Y;
			}

			fDist = vDir.normalizeLen();
			float curDist = fDist * ( (mFlyTime == 0) ? 1.0f : ((float)mEscapTime / (float)mFlyTime) );
			mStartCur = mStartPos;
			mEndCur = mStartCur + vDir * curDist;
			...
			randomOri.rotateAxis(vDir, randAngle1);
			Vector3 vUp = randomOri * calcVertVec(vDir);
			randomOri.rotateAxis(vUp, randAngle2*mDeviation);
			Vector3 vNewDir = randomOri * vDir;

			Vector3 rotEnd = mStartCur + vNewDir * curDist;
			Vector3 vStart(mStartCur * mat);
			Vector3 vEnd(rotEnd * mat);
		...
		for (int i = 0; i < nCount; i++)
		{
			...
			{
				Quaternion ori;
				ori.rotateAxis(mSplineIntDir[i+1], mpRandomData[1][i]);
	...
	void EffectLayerLighting::_updateRenderQueue()
	{
		if ((!mbAffected) || (!mbVisible))
			return;

		EffectRenderable* pRenderable = EffectSystemManager::Instance()->getRenderable(mMaterial->getTexturePtr(), mRenderGroup, mMaterial->getBlendMode(), mMaterial, (mSegment + 1) * 2 * mLightNum, mSegment * 6 * mLightNum, mViewSpace);
		if(!pRenderable)
			return;
		...
		if(pRenderable->getLockPtr((mSegment+1)*2*mLightNum, mSegment*6*mLightNum, vertBegin, pVertexPtr, pIdxLockPtr))
		{
			...
			for (int i = 0; i < nRenderCount; i++)
			{
				...
				else
				{
					...
					{
						Quaternion q;
						q.rotateAxis(vDir, mpRandomData[1][nCount]);
	...
	void EffectLayerLighting::mergeAligenBox(Box& aabb)
	{
		...
		else
		{
			...
			aabb.addPoint(mEndCur/*mEndPos*/);
			aabb.addPoint(mStartPos + startNoise);
			aabb.addPoint(mStartPos - startNoise);
			aabb.addPoint(mEndCur/*mEndPos*/ + endNoise);
			aabb.addPoint(mEndCur/*mEndPos*/ - endNoise);
