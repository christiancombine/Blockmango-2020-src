...
namespace LORD
{
	...
	bool PathUtil::IsPathExist(const String& path, size_t* size /* = nullptr */)
	{
		struct stat tagStat;
		int ret = stat(path.c_str(), &tagStat);
		if (ret == 0 && size != nullptr)
	...
	bool PathUtil::CopyFileOrDir(const String& srcDir, const String& destDir)
	{
		struct stat st;
		if (stat(srcDir.c_str(), &st) == -1)
			return false;

		bool flag = false;
		if (S_ISDIR(st.st_mode))
		{
			flag = CopyDir(srcDir, destDir);
		}
		else
		{
			flag = CopyFilePath(srcDir, destDir);
		}
		return flag;
	}
