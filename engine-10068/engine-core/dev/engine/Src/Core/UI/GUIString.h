...
/*!
\brief
	GUIString class used within the GUI system.

	For the most part, this class can replace std::string in basic usage.  However, currently GUIString does not use the
	current locale, and also comparisons do not take into account the Unicode data tables, so are not 'correct'
	as such.
*/
class LORD_CORE_API GUIString
{
public:
	/*************************************************************************
		Integral Types
	*************************************************************************/
	typedef		utf16			value_type;					//!< Basic 'code point' type used for GUIString (utf16)
	...
	typedef		utf16&			reference;					//!< Type used for utf16 code point references
	typedef		const utf16&	const_reference;			//!< Type used for constant utf16 code point references
	typedef		utf16*			pointer;					//!< Type used for utf16 code point pointers
	typedef		const utf16*	const_pointer;				//!< Type used for constant utf16 code point pointers
...
private:
	...
	*************************************************************************/
	...
	utf16		d_quickbuff[STR_QUICKBUFF_SIZE];	//!< This is a integrated 'quick' buffer to save allocations for smallish strings
	utf16*		d_buffer;							//!< Pointer the the main buffer memory.  This is only valid when quick-buffer is not being used

public:
	/*************************************************************************
		Iterator Classes
	*************************************************************************/
    //! regular iterator for GUIString.
    class iterator : public std::iterator<std::random_access_iterator_tag, utf16>
    {
    public:
        iterator() : d_ptr(0) {}
        explicit iterator(utf16* const ptr) : d_ptr(ptr) {}
        ...
        GUIString::iterator& operator++()
        {
            ++d_ptr;
            return *this;
        }

        GUIString::iterator operator++(int)
        {
            GUIString::iterator temp = *this;
            ++*this;
            return temp;
        }
        ...
        GUIString::iterator& operator+=(difference_type offset)
        {
            d_ptr += offset;
            return *this;
        }

        GUIString::iterator operator+(difference_type offset) const
        {
            GUIString::iterator temp = *this;
            return temp += offset;
        }
        ...
        utf16& operator[](difference_type offset) const
        {
            return *(*this + offset);
        }

        friend difference_type operator-(const GUIString::iterator& lhs,
                                         const GUIString::iterator& rhs)
            { return lhs.d_ptr - rhs.d_ptr; }

        friend GUIString::iterator operator+(difference_type offset, const GUIString::iterator& iter)
            { return iter + offset; }

        friend bool operator==(const GUIString::iterator& lhs,
                               const GUIString::iterator& rhs)
            { return lhs.d_ptr == rhs.d_ptr; }
    ...
    //! const iterator for GUIString.
    class const_iterator : public std::iterator<std::random_access_iterator_tag, const utf16>
    {
    public:
        const_iterator() : d_ptr(0) {}
        explicit const_iterator(const utf16* const ptr) : d_ptr(ptr) {}
        const_iterator(const GUIString::iterator& iter) : d_ptr(iter.d_ptr) {}

        const utf16& operator*() const
        {
            return *d_ptr;
        }
        ...
        GUIString::const_iterator& operator++()
        {
            ++d_ptr;
            return *this;
        }

        GUIString::const_iterator operator++(int)
        {
            GUIString::const_iterator temp = *this;
            ++*this;
            return temp;
        }
        ...
        GUIString::const_iterator& operator+=(difference_type offset)
        {
            d_ptr += offset;
            return *this;
        }

        GUIString::const_iterator operator+(difference_type offset) const
        {
            GUIString::const_iterator temp = *this;
            return temp += offset;
        }
        ...
        const utf16& operator[](difference_type offset) const
        {
            return *(*this + offset);
        }

        GUIString::const_iterator& operator=(const GUIString::iterator& iter)
        {
            d_ptr = iter.d_ptr;
            return *this;
        }

        friend GUIString::const_iterator operator+(difference_type offset, const GUIString::const_iterator& iter)
            { return iter + offset; }

        friend difference_type operator-(const GUIString::const_iterator& lhs,
                                         const GUIString::const_iterator& rhs)
            { return lhs.d_ptr - rhs.d_ptr; }

        friend bool operator==(const GUIString::const_iterator& lhs,
                               const GUIString::const_iterator& rhs)
            { return lhs.d_ptr == rhs.d_ptr; }
	...
	/*!
	\brief
		Constant reverse iterator class for GUIString objects
	*/
	...
	/*!
	\brief
		Reverse iterator class for GUIString objects
	*/
#if defined(_MSC_VER) && ((_MSC_VER <= 1200) || ((_MSC_VER <= 1300) && defined(_STLPORT_VERSION)))
...
public:
    /*!
    \brief
        Functor that can be used as comparator in a std::map with GUIString keys.
        It's faster than using the default, but the map will no longer be sorted alphabetically.
    */
    struct FastLessCompare
    {
        bool operator() (const GUIString& a, const GUIString& b) const
        {
            const size_t la = a.length();
            const size_t lb = b.length();
            if (la == lb)
                return (memcmp(a.ptr(), b.ptr(), la*sizeof(utf16)) < 0);
            return (la < lb);
        }
    };
...
public:
	...
	*/
	...
	/*!
	\brief
		Destructor for GUIString objects
	*/
	~GUIString(void);

	//////////////////////////////////////////////////////////////////////////
	// Construction via CEGUI::GUIString
	//////////////////////////////////////////////////////////////////////////
	/*!
	\brief
	...
	\param str
		GUIString object used to initialise the newly created string
	...
	*/
	...
	/*!
	\brief
		Constructs a new string initialised with code points from another GUIString object.

	\param str
		GUIString object used to initialise the newly created string

	\param str_idx
		Starting code-point of \a str to be used when initialising the new GUIString

	\param str_num
		Maximum number of code points from \a str that are to be assigned to the new GUIString

	\return
		Nothing
	*/
	GUIString(const GUIString& str, size_type str_idx, size_type str_num = npos)
	{
		init();
	...
	\param std_str
		The std::string object that is to be used to initialise the new GUIString object.
	...
	\exception std::length_error	Thrown if resulting GUIString object would be too big.
	*/
	GUIString(const std::string& std_str)
	{
		init();
		assign(std_str);
	}
	...
	\param str_idx
		Starting character of \a std_str to be used when initialising the new GUIString
	...
	\param str_num
		Maximum number of characters from \a std_str that are to be assigned to the new GUIString
	...
	\exception std::length_error	Thrown if resulting GUIString object would be too big.
	*/
	GUIString(const std::string& std_str, size_type str_idx, size_type str_num = npos)
	{
		init();
	...
	/*!
	\brief
		Constructs a new GUIString object and initialise it using the provided utf8 encoded string buffer.
	...
	\note
		...
		would result in incorrect behaviour as the GUIString will attempt to 'decode' the data, with unpredictable
		results.
	...
	*/
	...
	\exception std::length_error	Thrown if resulting GUIString object would be too big.
	*/
	GUIString(const utf8* utf8_str, size_type chars_len)
	{
		init();
	...
	/*!
	\brief
		Constructs a new GUIString that is initialised with the specified code point

	\param num
		The number of times \a code_point is to be put into new GUIString object

	\param code_point
		The Unicode code point to be used when initialising the GUIString object
	...
	\exception std::length_error	Thrown if resulting GUIString object would be too big.
	*/
	GUIString(size_type num, utf16 code_point)
	{
		init();
	...
	\param beg
		Iterator describing the start of the data to be used when initialising the GUIString object

	\param end
		Iterator describing the (exclusive) end of the data to be used when initialising the GUIString object

	\return
		Nothing
	*/
	GUIString(const_iterator iter_beg, const_iterator iter_end)
	{
		init();
	...
	/*!
	\brief
		Constructs a new GUIString object and initialise it using the provided c-string.
	...
	*/
	...
	// for UIEditor int from wchar
	GUIString(const wchar_t* strWChar, size_type nWCharCount)
	{
		init();
		grow(nWCharCount);
		utf16* pt = ptr();

		for (size_type i = 0; i < nWCharCount; ++i)
		{
			*pt++ = (utf16)*strWChar++;
		}
	...
	/*!
	\brief
		Constructs a new GUIString object and initialise it using characters from the provided char array.
	...
	\exception std::length_error	Thrown if resulting GUIString object would be too big.
	*/
	//GUIString(const char* chars, size_type chars_len)
	//{
	//	init();
	...
	/*!
	\brief
		Returns the size of the GUIString in code points

	\return
		Number of code points currently in the GUIString
	*/
	...
	*/
	...
	/*!
	\brief
		Returns true if the GUIString is empty

	\return
		true if the GUIString is empty, else false.
	*/
	...
	// 根据unicode返回char的个数
	int		charCount(utf16 c) const
	{
		if (c >= 0x4E00 && c <= 0x9FCC)
			return 2;

		//guoshilei(国石磊) 10-30 16:32:56
		//不连续可以吗?
		//ASCII码范围是:1-8,11-12,14-127
	...
	GUIString	cutGBKByByteCount(int byteCount) const
	{
		int count = 0;
		size_type len = d_cplength;
		const utf16* buf = ptr();
		...
		while (len--)
		{
			...
			if (count > byteCount)
			{
				return substr(0, d_cplength-len-1);
			}
		}
	...
	/*!
	\brief
		Returns the maximum size of a GUIString.

		Any operation that would result in a GUIString that is larger than this value will throw the std::length_error exception.
	...
	*/
	...
	{
		return (((size_type)-1) / sizeof(utf16));
	}
	...
	/*!
	\brief
		Return the number of code points that the GUIString could hold before a re-allocation would be required.

	\return
		Size of the current reserve buffer.  This is the maximum number of code points the GUIString could hold before a buffer
	...
	*/
	...
	\param num
		...
		GUIString size (operation is currently not implemented).  If \a num is 0, then the buffer is re-allocated to fit the current GUIString size.
	...
	*/
	...
	/*!
	\brief
		Compares this GUIString with the GUIString 'str'.

	\note
		This does currently not properly consider Unicode and / or the system locale.

	\param str
		The GUIString object that is to compared with this GUIString.

	\return
		-  0 if the GUIString objects are equal
		- <0 if this GUIString is lexicographically smaller than \a str
		- >0 if this GUIString is lexicographically greater than \a str
	*/
	int		compare(const GUIString& str) const
	{
	...
	/*!
	\brief
		Compares code points from this GUIString with code points from the GUIString 'str'.

	\note
		This does currently not properly consider Unicode and / or the system locale.

	\param idx
		Index of the first code point from this GUIString to consider.

	\param len
		Maximum number of code points from this GUIString to consider.

	\param str
		The GUIString object that is to compared with this GUIString.

	\param str_idx
		Index of the first code point from GUIString \a str to consider.

	\param str_len
		Maximum number of code points from GUIString \a str to consider
	...
	*/
	int		compare(size_type idx, size_type len, const GUIString& str, size_type str_idx = 0, size_type str_len = npos) const
	{
		if ((d_cplength < idx) || (str.d_cplength < str_idx))
			throw(std::out_of_range("Index is out of range for CEGUI::GUIString"));
	...
	/*!
	\brief
		Compares this GUIString with the std::string 'std_str'.

	\note
		This does currently not properly consider Unicode and / or the system locale.

	\param std_str
		The std::string object that is to compared with this GUIString.
	...
	*/
	...
	/*!
	\brief
		Compares code points from this GUIString with code points from the std::string 'std_str'.
	...
	*/
	...
	/*!
	\brief
		Compares this GUIString with the null-terminated utf8 encoded 'utf8_str'.

	\note
		This does currently not properly consider Unicode and / or the system locale.

	\param utf8_str
		The buffer containing valid Unicode data encoded as utf8 that is to compared with this GUIString.
	...
	*/
	...
	/*!
	\brief
		Compares code points from this GUIString with the null-terminated utf8 encoded 'utf8_str'.
	...
	*/
	...
	/*!
	\brief
		Compares code points from this GUIString with the utf8 encoded data in buffer 'utf8_str'.
	...
	*/
	...
	/*!
	\brief
		Compares this GUIString with the given c-string.

	\note
		This does currently not properly consider Unicode and / or the system locale.

	\param c_str
		The c-string that is to compared with this GUIString.
	...
	*/
	...
	/*!
	\brief
		Compares code points from this GUIString with the given c-string.
	...
	*/
	...
	/*!
	\brief
		Compares code points from this GUIString with chars in the given char array.
	...
	\param chars
		The array containing the chars that are to compared with this GUIString.
	...
	*/
	...
	\note
		- For constant strings length()/size() provide a valid index and will access the default utf16 value.
	...
	\return
		The utf16 code point at the given index within the GUIString.
	*/
	...
	*/
	...
	/*!
	\brief
		Returns contents of the GUIString as a null terminated string of utf8 encoded data.

	\return
		Pointer to a char buffer containing the contents of the GUIString encoded as null-terminated utf8 data.

	\note
		The buffer returned from this function is owned by the GUIString object.

	\note
		Any function that modifies the GUIString data will invalidate the buffer returned by this call.
	*/
	...
	/*!
	\brief
		Returns contents of the GUIString as utf8 encoded data.

	\return
		Pointer to a buffer containing the contents of the GUIString encoded utf8 data.
    ...
    */
	...
	/*!
	\brief
		Copies an area of the GUIString into the provided buffer as encoded utf8 data.

	\param buf
		Pointer to a buffer that is to receive the encoded data (this must be big enough to hold the encoded data)

	\param len
		Maximum number of code points from the GUIString that should be encoded into the buffer
	...
	\return
		...
		\note A code unit does not equal a code point.  A utf16 code point, when encoded as utf8, can occupy between 1 and 4 code units.

	\exception std::out_of_range	Thrown if \a idx was invalid for this GUIString.
	*/
	...
	/*!
	\brief
		Return the number of utf8 code units required to hold an area of the GUIString when encoded as utf8 data
	...
	*/
	size_type	utf8_stream_len(size_type num = npos, size_type idx = 0) const
	{
		...
		if (d_cplength < idx)
			throw(out_of_range("Index was out of range for CEGUI::GUIString object"));

		size_type	maxlen = d_cplength - idx;

		return encoded_size(&ptr()[idx], Math::Min(num, maxlen));
	}
	...
	/*!
	\brief
		Assign the value of GUIString \a str to this GUIString

	\param str
		GUIString object containing the string value to be assigned.

	\return
		This GUIString after the assignment has happened
	*/
	...
	/*!
	\brief
		Assign a sub-string of GUIString \a str to this GUIString

	\param str
		GUIString object containing the string data to be assigned.
	...
	*/
	GUIString&	assign(const GUIString& str, size_type str_idx = 0, size_type str_num = npos)
	{
		if (str.d_cplength < str_idx)
			throw(std::out_of_range("Index was out of range for CEGUI::GUIString object"));
		...
		grow(str_num);
		setlen(str_num);
		memcpy(ptr(), &str.ptr()[str_idx], str_num * sizeof(utf16));
	...
	/*!
	\brief
		Assign the value of std::string \a std_str to this GUIString
	...
	\exception std::length_error	Thrown if the resulting GUIString would have been too large.
	*/
	GUIString&	operator=(const std::string& std_str)
	{
	...
	/*!
	\brief
		Assign a sub-string of std::string \a std_str to this GUIString
	...
	\exception std::length_error	Thrown if the resulting GUIString would have been too large.
	*/
	GUIString&	assign(const std::string& std_str, size_type str_idx = 0, size_type str_num = npos)
	{
		if (std_str.size() < str_idx)
		...
		while(str_num--)
		{
			((*this)[str_num]) = static_cast<utf16>(static_cast<unsigned char>(std_str[str_num + str_idx]));
		}
	...
	/*!
	\brief
		Assign to this GUIString the string value represented by the given null-terminated utf8 encoded data
	...
	\exception std::length_error	Thrown if the resulting GUIString would have been too large.
	*/
	GUIString&	operator=(const utf8* utf8_str)
	{
	...
	\exception std::length_error	Thrown if the resulting GUIString would have been too large.
	*/
	GUIString&	assign(const utf8* utf8_str)
	{
		return assign(utf8_str, utf_length(utf8_str));
	}
	...
	/*!
	\brief
		Assign to this GUIString the string value represented by the given utf8 encoded data
	...
	\exception std::length_error	Thrown if the resulting GUIString would have been too large, or if str_num is 'npos'.
	*/
	GUIString&	assign(const utf8* utf8_str, size_type str_num)
	{
		if (str_num == npos)
			throw(std::length_error("Length for utf8 encoded string can not be 'npos'"));
	...
	/*!
	\brief
		Assigns the specified utf16 code point to this GUIString.  Result is always a GUIString 1 code point in length.

	\param code_point
		Valid utf16 Unicode code point to be assigned to the string

	\return
		This GUIString after assignment
	*/
	GUIString&	operator=(utf16 code_point)
	{
	...
	/*!
	\brief
		Assigns the specified code point repeatedly to the GUIString
	...
	*/
	GUIString&	assign(size_type num, utf16 code_point)
	{
		if (num == npos)
	...
	/*!
	\brief
		Assign to this GUIString the given C-string.
	...
	\exception std::length_error	Thrown if the resulting GUIString would have been too large.
	*/
	GUIString&	operator=(const char* cstr)
	{
		utf8* p = (utf8*)cstr;
		return assign(p, utf_length(p));
	}
	...
	\exception std::length_error	Thrown if the resulting GUIString would have been too large.
	*/
	//GUIString&	assign(const char* cstr)
	//{
	...
	/*!
	\brief
		Assign to this GUIString a number of chars from a char array.
	...
	\exception std::length_error	Thrown if the resulting GUIString would have been too large.
	*/
	//GUIString&	assign(const char* chars, size_type chars_len)
	//{
	//	grow(chars_len);
	//	utf16* pt = ptr();
	...
	//	{
	//		*pt++ = static_cast<utf16>(static_cast<unsigned char>(*chars++));
	//	}
	...
	/*!
	\brief
		Swaps the value of this GUIString with the given GUIString \a str

	\param str
		GUIString object whos value is to be swapped with this GUIString.

	\return
		Nothing
	*/
	void	swap(GUIString& str)
	{
		...
		{
			utf16		temp_qbf[STR_QUICKBUFF_SIZE];

			memcpy(temp_qbf, d_quickbuff, STR_QUICKBUFF_SIZE * sizeof(utf16));
			memcpy(d_quickbuff, str.d_quickbuff, STR_QUICKBUFF_SIZE * sizeof(utf16));
			memcpy(str.d_quickbuff, temp_qbf, STR_QUICKBUFF_SIZE * sizeof(utf16));
		}
	...
	/*!
	\brief
		Appends the GUIString \a str

	\param str
		GUIString object that is to be appended

	\return
		This GUIString after the append operation

	\exception std::length_error	Thrown if resulting GUIString would be too large.
	*/
	GUIString&	operator+=(const GUIString& str)
	{
		return append(str);
	...
	/*!
	\brief
		Appends a sub-string of the GUIString \a str

	\param str
		GUIString object containing data to be appended
	...
	\exception std::length_error	Thrown if resulting GUIString would be too large.
	*/
	GUIString& append(const GUIString& str, size_type str_idx = 0, size_type str_num = npos)
	{
		if (str.d_cplength < str_idx)
			throw(std::out_of_range("Index is out of range for CEGUI::GUIString"));
		...
		memcpy(&ptr()[d_cplength], &str.ptr()[str_idx], str_num * sizeof(utf16));
	...
	*/
	...
	\exception std::length_error	Thrown if resulting GUIString would be too large.
	*/
	GUIString& append(const std::string& std_str, size_type str_idx = 0, size_type str_num = npos)
	{
		if (std_str.size() < str_idx)
			throw(std::out_of_range("Index is out of range for std::string"));
		...
		grow(newsze);
		utf16* pt = &ptr()[newsze-1];

		while(str_num--)
			*pt-- = static_cast<utf16>(static_cast<unsigned char>(std_str[str_num]));
	...
	/*!
	\brief
		Appends to the GUIString the null-terminated utf8 encoded data in the buffer utf8_str.
	...
	*/
	...
	\exception std::length_error	Thrown if resulting GUIString would be too large.
	*/
	GUIString& append(const utf8* utf8_str)
	{
		return append(utf8_str, utf_length(utf8_str));
	}
	...
	/*!
	\brief
		Appends to the GUIString the utf8 encoded data in the buffer utf8_str.
	...
	\exception std::length_error	Thrown if resulting GUIString would be too large, or if \a len was 'npos'
	*/
	GUIString& append(const utf8* utf8_str, size_type len)
	{
		if (len == npos)
			throw(std::length_error("Length for utf8 encoded string can not be 'npos'"));
	...
	\param code_point
		utf16 Unicode code point that is to be appended

	\return
		This GUIString after the append operation

	\exception std::length_error	Thrown if resulting GUIString would be too long.
	*/
	GUIString& operator+=(utf16 code_point)
	{
	...
	\exception std::length_error	Thrown if resulting GUIString would be too long, or if \a num was 'npos'.
	*/
	GUIString& append(size_type num, utf16 code_point)
	{
		if (num == npos)
			throw(std::length_error("Code point count can not be 'npos'"));
	...
	\exception std::length_error	Thrown if resulting GUIString would be too long.
	*/
	void	push_back(utf16 code_point)
	{
		append(1, code_point);
	}
	...
	*/
	GUIString&	append(const_iterator iter_beg, const_iterator iter_end)
	{
	...
	/*!
	\brief
		Appends to the GUIString the given c-string.
	...
	*/
	...
	\exception std::length_error	Thrown if resulting GUIString would be too large.
	*/
	//GUIString& append(const char* cstr)
	//{
	//	return append(cstr, strlen(cstr));
	//}
	...
	/*!
	\brief
		Appends to the GUIString chars from the given char array.
	...
	\exception std::length_error	Thrown if resulting GUIString would be too large, or if \a chars_len was 'npos'
	*/
	//GUIString& append(const char* chars, size_type chars_len)
	//{
	//	if (chars_len == npos)
	//		throw(std::length_error("Length for char array can not be 'npos'"));
	...
	//	utf16* pt = &ptr()[newsz-1];

	//	while(chars_len--)
	//		*pt-- = static_cast<utf16>(static_cast<unsigned char>(chars[chars_len]));
	...
	/*!
	\brief
		Inserts the given GUIString object at the specified position.

	\param idx
		Index where the string is to be inserted.

	\param str
		GUIString object that is to be inserted.

	\return
		This GUIString after the insert.

	\exception std::out_of_range	Thrown if \a idx is invalid for this GUIString.
	\exception std::length_error	Thrown if resulting GUIString would be too large.
	*/
	GUIString&	insert(size_type idx, const GUIString& str)
	{
	...
	/*!
	\brief
		Inserts a sub-string of the given GUIString object at the specified position.

	\param idx
		Index where the string is to be inserted.

	\param str
		GUIString object containing data to be inserted.
	...
	\exception std::length_error	Thrown if resulting GUIString would be too large.
	*/
	GUIString& insert(size_type idx, const GUIString& str, size_type str_idx, size_type str_num)
	{
		if ((d_cplength < idx) || (str.d_cplength < str_idx))
			throw(std::out_of_range("Index is out of range for CEGUI::GUIString"));
		...
		grow(newsz);
		memmove(&ptr()[idx + str_num], &ptr()[idx], (d_cplength - idx) * sizeof(utf16));
		memcpy(&ptr()[idx], &str.ptr()[str_idx], str_num * sizeof(utf16));
		setlen(newsz);
	...
	\exception std::length_error	Thrown if resulting GUIString would be too large.
	*/
	GUIString&	insert(size_type idx, const std::string& std_str)
	{
	...
	\exception std::length_error	Thrown if resulting GUIString would be too large.
	*/
	GUIString& insert(size_type idx, const std::string& std_str, size_type str_idx, size_type str_num)
	{
		if (d_cplength < idx)
			throw(std::out_of_range("Index is out of range for CEGUI::GUIString"));
		...
		utf16* pt = &ptr()[idx + str_num - 1];

		while(str_num--)
			*pt-- = static_cast<utf16>(static_cast<unsigned char>(std_str[str_idx + str_num]));
	...
	\exception std::length_error	Thrown if resulting GUIString would be too large.
	*/
	GUIString&	insert(size_type idx, const utf8* utf8_str)
	{
	...
	\exception std::out_of_range	Thrown if \a idx is invalid for this GUIString.
	\exception std::length_error	Thrown if resulting GUIString would be too large, or if \a len is 'npos'
	*/
	GUIString& insert(size_type idx, const utf8* utf8_str, size_type len)
	{
		if (d_cplength < idx)
			throw(std::out_of_range("Index is out of range for CEGUI::GUIString"));
		...
		grow(newsz);
		memmove(&ptr()[idx + encsz], &ptr()[idx], (d_cplength - idx) * sizeof(utf16));
	...
	/*!
	\brief
		Inserts a code point multiple times into the GUIString
	...
	\param code_point
		The utf16 code point that is to be inserted

	\return
		This GUIString after the insertion.

	\exception std::out_of_range	Thrown if \a idx is invalid for this GUIString.
	\exception std::length_error	Thrown if resulting GUIString would be too large, or if \a num is 'npos'
	*/
	GUIString& insert(size_type idx, size_type num, utf16 code_point)
	{
		if (d_cplength < idx)
			throw(std::out_of_range("Index is out of range for CEGUI::GUIString"));
		...
		memmove(&ptr()[idx + num], &ptr()[idx], (d_cplength - idx) * sizeof(utf16));

		utf16* pt = &ptr()[idx + num - 1];
	...
	\exception std::length_error	Thrown if resulting GUIString would be too large, or if \a num is 'npos'
	*/
	void insert(iterator pos, size_type num, utf16 code_point)
	{
	...
	/*!
	\brief
		Inserts a single code point into the GUIString
	...
	\exception std::length_error	Thrown if resulting GUIString would be too large.
	*/
	iterator insert(iterator pos, utf16 code_point)
	{
	...
	*/
	...
	\exception std::length_error	Thrown if resulting GUIString would be too large.
	*/
	//GUIString&	insert(size_type idx, const char* cstr)
	//{
	...
	\exception std::out_of_range	Thrown if \a idx is invalid for this GUIString.
	\exception std::length_error	Thrown if resulting GUIString would be too large, or if \a chars_len is 'npos'
	*/
	//GUIString& insert(size_type idx, const char* chars, size_type chars_len)
	//{
	//	if (d_cplength < idx)
	//		throw(std::out_of_range("Index is out of range for CEGUI::GUIString"));
	...
	//	grow(newsz);
	//	memmove(&ptr()[idx + chars_len], &ptr()[idx], (d_cplength - idx) * sizeof(utf16));

	//	utf16* pt = &ptr()[idx + chars_len - 1];
	...
	/*!
	\brief
		Removes all data from the GUIString
	...
	*/
	...
	\return
		The empty GUIString (*this)
	*/
	GUIString& erase(void)
	{
		clear();
		return *this;
	}
	...
	\return
		This GUIString after the erase operation

	\exception std::out_of_range	Thrown if \a idx is invalid for this GUIString.
	*/
	GUIString&	erase(size_type idx)
	{
		return erase(idx, 1);
	}
	...
	\exception std::out_of_range	Thrown if \a idx is invalid for this GUIString.
	*/
	GUIString& erase(size_type idx, size_type len)
	{
		...
		memmove(&ptr()[idx], &ptr()[idx + len], (d_cplength - idx - len) * sizeof(utf16));
		setlen(newsz);
		return	*this;
	}
	...
	*/
	...
	\return
		This GUIString after the erase operation.
	*/
	GUIString& erase(iterator iter_beg, iterator iter_end)
	{
	...
	/*!
	\brief
		Resizes the GUIString either by inserting default utf16 code points to make it larger, or by truncating to make it smaller

	\param num
		The length, in code points, that the GUIString is to be made.

	\return
		Nothing.

	\exception std::length_error	Thrown if the GUIString would be too large.
	*/
	...
	/*!
	\brief
		Resizes the GUIString either by inserting the given utf16 code point to make it larger, or by truncating to make it smaller

	\param num
		The length, in code points, that the GUIString is to be made.

	\param code_point
		The utf16 code point that should be used when majing the GUIString larger
	...
	\exception std::length_error	Thrown if the GUIString would be too large.
	*/
	void	resize(size_type num, utf16 code_point)
	{
		if (num < d_cplength)
		{
			setlen(num);
	...
	/*!
	\brief
		Replace code points in the GUIString with the specified GUIString object
	...
	\param str
		The GUIString object that is to replace the specified code points

	\return
		This GUIString after the replace operation

	\exception std::out_of_range	Thrown if \a idx is invalid for this GUIString
	\exception std::length_error	Thrown if the resulting GUIString would be too large.
	*/
	GUIString& replace(size_type idx, size_type len, const GUIString& str)
	{
	...
	/*!
	\brief
		Replace the code points in the range [beg, end) with the specified GUIString object
	...
	\param str
		The GUIString object that is to replace the specified range of code points
	...
	\exception std::length_error	Thrown if the resulting GUIString would be too large.
	*/
	GUIString& replace(iterator iter_beg, iterator iter_end, const GUIString& str)
	{
	...
	/*!
	\brief
		Replace code points in the GUIString with a specified sub-string of a given GUIString object.
	...
	\param str
		GUIString object containing the data that will replace the specified range of code points
	...
	\exception std::length_error	Thrown if the resulting GUIString would have been too large.
	*/
	GUIString& replace(size_type idx, size_type len, const GUIString& str, size_type str_idx, size_type str_num)
	{
		if ((d_cplength < idx) || (str.d_cplength < str_idx))
			throw(std::out_of_range("Index is out of range for CEGUI::GUIString"));
		...
		if ((idx + len) < d_cplength)
			memmove(&ptr()[idx + str_num], &ptr()[len + idx], (d_cplength - idx - len) * sizeof(utf16));
	...
	/*!
	\brief
		Replace code points in the GUIString with the specified std::string object
	...
	\exception std::length_error	Thrown if the resulting GUIString would be too large.
	*/
	GUIString& replace(size_type idx, size_type len, const std::string& std_str)
	{
	...
	\exception std::length_error	Thrown if the resulting GUIString would be too large.
	*/
	GUIString& replace(iterator iter_beg, iterator iter_end, const std::string& std_str)
	{
	...
	/*!
	\brief
		Replace code points in the GUIString with a specified sub-string of a given std::string object.
	...
	\exception std::length_error	Thrown if the resulting GUIString would have been too large.
	*/
	GUIString& replace(size_type idx, size_type len, const std::string& std_str, size_type str_idx, size_type str_num)
	{
		if (d_cplength < idx)
			throw(std::out_of_range("Index is out of range for CEGUI::GUIString"));
	...
	/*!
	\brief
		Replace code points in the GUIString with the specified null-terminated utf8 encoded data.
	...
	\exception std::length_error	Thrown if the resulting GUIString would be too large.
	*/
	GUIString& replace(size_type idx, size_type len, const utf8* utf8_str)
	{
	...
	\exception std::length_error	Thrown if the resulting GUIString would be too large.
	*/
	GUIString& replace(iterator iter_beg, iterator iter_end, const utf8* utf8_str)
	{
	...
	/*!
	\brief
		Replace code points in the GUIString with the specified utf8 encoded data.
	...
	\exception std::out_of_range	Thrown if \a idx is invalid for this GUIString
	\exception std::length_error	Thrown if the resulting GUIString would be too large, or if \a str_len was 'npos'.
	*/
	GUIString& replace(size_type idx, size_type len, const utf8* utf8_str, size_type str_len)
	{
		if (d_cplength < idx)
			throw(std::out_of_range("Index is out of range for CEGUI::GUIString"));
		...
		if ((idx + len) < d_cplength)
			memmove(&ptr()[idx + encsz], &ptr()[len + idx], (d_cplength - idx - len) * sizeof(utf16));
	...
	\return
		...
		\exception std::length_error	Thrown if the resulting GUIString would be too large, or if \a str_len was 'npos'.
	*/
	GUIString& replace(iterator iter_beg, iterator iter_end, const utf8* utf8_str, size_type str_len)
	{
	...
	\exception std::out_of_range	Thrown if \a idx is invalid for this GUIString
	\exception std::length_error	Thrown if resulting GUIString would have been too long, or if \a num was 'npos'.
	*/
	GUIString& replace(size_type idx, size_type len, size_type num, utf16 code_point)
	{
		if (d_cplength < idx)
			throw(std::out_of_range("Index is out of range for CEGUI::GUIString"));
		...
		if ((idx + len) < d_cplength)
			memmove(&ptr()[idx + num], &ptr()[len + idx], (d_cplength - idx - len) * sizeof(utf16));
	...
	\exception std::length_error	Thrown if resulting GUIString would have been too long, or if \a num was 'npos'.
	*/
	GUIString& replace(iterator iter_beg, iterator iter_end, size_type num, utf16 code_point)
	{
	...
	\return
		This GUIString after the insert operation.

	\exception std::length_error	Thrown if the resulting string would be too long.
	*/
	GUIString& replace(iterator iter_beg, iterator iter_end, const_iterator iter_newBeg, const_iterator iter_newEnd)
	{
		...
		else
		{
			...
			if ((idx + len) < d_cplength)
				memmove(&ptr()[idx + str_len], &ptr()[len + idx], (d_cplength - idx - len) * sizeof(utf16));

			memcpy(&ptr()[idx], iter_newBeg.d_ptr, str_len * sizeof(utf16));
			setlen(newsz);
		}
	...
	/*!
	\brief
		Replace code points in the GUIString with the specified c-string.
	...
	\exception std::length_error	Thrown if the resulting GUIString would be too large.
	*/
	//GUIString& replace(size_type idx, size_type len, const char* cstr)
	//{
	...
	\exception std::length_error	Thrown if the resulting GUIString would be too large.
	*/
	//GUIString& replace(iterator iter_beg, iterator iter_end, const char* cstr)
	//{
	...
	/*!
	\brief
		Replace code points in the GUIString with chars from the given char array.
	...
	\exception std::out_of_range	Thrown if \a idx is invalid for this GUIString
	\exception std::length_error	Thrown if the resulting GUIString would be too large, or if \a chars_len was 'npos'.
	*/
	//GUIString& replace(size_type idx, size_type len, const char* chars, size_type chars_len)
	//{
	//	if (d_cplength < idx)
	//		throw(std::out_of_range("Index is out of range for CEGUI::GUIString"));
	...
	//	if ((idx + len) < d_cplength)
	//		memmove(&ptr()[idx + chars_len], &ptr()[len + idx], (d_cplength - idx - len) * sizeof(utf16));
	...
	\exception std::length_error	Thrown if the resulting GUIString would be too large, or if \a chars_len was 'npos'.
	*/
	//GUIString& replace(iterator iter_beg, iterator iter_end, const char* chars, size_type chars_len)
	//{
	...
	\param code_point
		The utf16 code point to search for
	...
	*/
	size_type	find(utf16 code_point, size_type idx = 0) const
	{
		if (idx < d_cplength)
		{
			const utf16* pt = &ptr()[idx];
	...
	\return
		...
		- npos if the code point could not be found
	*/
	size_type	rfind(utf16 code_point, size_type idx = npos) const
	{
		if (idx >= d_cplength)
			idx = d_cplength - 1;
	...
	\param str
		GUIString object describing the sub-string to search for
	...
	*/
	size_type	find(const GUIString& str, size_type idx = 0) const
	{
	...
	\return
		...
		- npos if the sub-string could not be found
	*/
	size_type	rfind(const GUIString& str, size_type idx = npos) const
	{
		if (str.d_cplength == 0)
	...
	*/
	...
	\param str
		GUIString object describing the set of code points.
	...
	*/
	size_type	find_first_of(const GUIString& str, size_type idx = 0) const
	{
		if (idx < d_cplength)
		{
			const utf16* pt = &ptr()[idx];
	...
	*/
	size_type	find_first_not_of(const GUIString& str, size_type idx = 0) const
	{
		if (idx < d_cplength)
		{
			const utf16* pt = &ptr()[idx];
	...
	*/
	...
	\return
		...
		- npos if the code point could not be found
	*/
	size_type	find_first_of(utf16 code_point, size_type idx = 0) const
	{
	...
	\exception std::out_of_range	Thrown if \a idx is invalid for this GUIString.
	*/
	size_type	find_first_not_of(utf16 code_point, size_type idx = 0) const
	{
		if (idx < d_cplength)
		{
			do
	...
	*/
	...
	\return
		...
		- npos if none of the code points in \a str were found.
	*/
	size_type	find_last_of(const GUIString& str, size_type idx = npos) const
	{
		if (d_cplength > 0)
		{
			if (idx >= d_cplength)
	...
	\return
		...
		- npos if all code points matched one of the code points in \a str.
	*/
	size_type	find_last_not_of(const GUIString& str, size_type idx = npos) const
	{
		if (d_cplength > 0)
		{
			if (idx >= d_cplength)
	...
	*/
	...
	\return
		...
		- npos if the code point could not be found
	*/
	size_type	find_last_of(utf16 code_point, size_type idx = npos) const
	{
	...
	\return
		...
		- npos if all code points matched \a code_point
	*/
	size_type	find_last_not_of(utf16 code_point, size_type idx = npos) const
	{
		if (d_cplength > 0)
		{
			if (idx >= d_cplength)
	...
	*/
	...
	/*!
	\brief
		Returns a substring of this GUIString.
	...
	\return
		A GUIString object containing the specified sub-string.

	\exception std::out_of_range	Thrown if \a idx is invalid for this GUIString.
	*/
	GUIString	substr(size_type idx = 0, size_type len = npos) const
	{
		if (d_cplength < idx)
			throw(std::out_of_range("Index is out of range for this CEGUI::GUIString"));

		return GUIString(*this, idx, len);
	}
	...
	/*!
	\brief
		Return a forwards iterator that describes the beginning of the GUIString

	\return
		iterator object that describes the beginning of the GUIString.
	*/
	iterator		begin(void)
	{
		return iterator(ptr());
	...
	/*!
	\brief
		Return a constant forwards iterator that describes the beginning of the GUIString

	\return
		const_iterator object that describes the beginning of the GUIString.
	*/
	...
	/*!
	\brief
		Return a forwards iterator that describes the end of the GUIString

	\return
		iterator object that describes the end of the GUIString.
	*/
	iterator		end(void)
	{
	...
	/*!
	\brief
		Return a constant forwards iterator that describes the end of the GUIString

	\return
		const_iterator object that describes the end of the GUIString.
	*/
	...
	/*!
	\brief
		Return a reverse iterator that describes the beginning of the GUIString

	\return
		reverse_iterator object that describes the beginning of the GUIString (so is actually at the end)
	*/
	...
	/*!
	\brief
		Return a constant reverse iterator that describes the beginning of the GUIString

	\return
		const_reverse_iterator object that describes the beginning of the GUIString (so is actually at the end)
	*/
	...
	/*!
	\brief
		Return a reverse iterator that describes the end of the GUIString

	\return
		reverse_iterator object that describes the end of the GUIString (so is actually at the beginning)
	*/
	...
	/*!
	\brief
		Return a constant reverse iterator that describes the end of the GUIString

	\return
		const_reverse_iterator object that describes the end of the GUIString (so is actually at the beginning)
	*/
...
private:
	...
	*************************************************************************/
	...
	size_type encode(const utf16* src, utf8* dest, size_type dest_len, size_type src_len = 0) const
	{
	...
	size_type encode(const utf8* src, utf16* dest, size_type dest_len, size_type src_len = 0) const
	{
		// count length for null terminated source...
		if (src_len == 0)
		{
	...
	// return the number of utf8 code units required to encode the given utf16 code point
	size_type encoded_size(utf16 code_point) const
	{
		if (code_point < 0x80)
			return 1;
	...
	// return number of code units required to re-encode given null-terminated utf16 data as utf8.  return does not include terminating null.
	size_type encoded_size(const utf16* buf) const
	{
		return encoded_size(buf, utf_length(buf));
	}

	// return number of code units required to re-encode given utf16 data as utf8.   len is number of code units in 'buf'.
	size_type encoded_size(const utf16* buf, size_type len) const
	{
		size_type count = 0;
	...
	// return number of utf16 code units required to re-encode given utf8 data as utf16.  return does not include terminating null.
	size_type encoded_size(const utf8* buf) const
	{
		return encoded_size(buf, utf_length(buf));
	}

	// return number of utf16 code units required to re-encode given utf8 data as utf16.  len is number of code units in 'buf'.
	...
	// return number of code units in a null terminated string
	size_type utf_length(const utf16* utf32_str) const
	{
		size_type cnt = 0;
		while (*utf32_str++)
			cnt++;
	...
	// compare two utf16 buffers
	int	utf32_comp_utf32(const utf16* buf1, const utf16* buf2, size_type cp_count) const
	{
		if (!cp_count)
			return 0;
	...
	// compare utf16 buffer with char buffer (chars are taken to be code-points in the range 0x00-0xFF)
	//int utf32_comp_char(const utf16* buf1, const char* buf2, size_type cp_count) const
	//{
	//	if (!cp_count)
	//		return 0;

	//	while ((--cp_count) && (*buf1 == static_cast<utf16>(static_cast<unsigned char>(*buf2))))
	//		buf1++, buf2++;

	//	return *buf1 - static_cast<utf16>(static_cast<unsigned char>(*buf2));
	//}

	// compare utf16 buffer with encoded utf8 data
	int utf32_comp_utf8(const utf16* buf1, const utf8* buf2, size_type cp_count) const
	{
		if (!cp_count)
			return 0;
	...
	size_type find_codepoint(const std::string& str, utf16 code_point) const
	{
		size_type idx = 0, sze = (size_type)str.size();

		while (idx != sze)
		{
			if (code_point == static_cast<utf16>(static_cast<unsigned char>(str[idx])))
				return idx;
	...
	size_type find_codepoint(const utf8* str, size_type len, utf16 code_point) const
	{
		size_type idx = 0;
	...
	//size_type find_codepoint(const char* chars, size_type chars_len, utf16 code_point) const
	//{
	//	for (size_type idx = 0; idx != chars_len; ++idx)
	//	{
	//		if (code_point == static_cast<utf16>(static_cast<unsigned char>(chars[idx])))
	//			return idx;
	//	}
...
/*!
\brief
	Return true if GUIString \a str1 is equal to GUIString \a str2
*/
bool LORD_CORE_API	operator==(const GUIString& str1, const GUIString& str2);

/*!
\brief
	Return true if GUIString \a str is equal to std::string \a std_str
*/
bool LORD_CORE_API	operator==(const GUIString& str, const std::string& std_str);

/*!
\brief
	Return true if GUIString \a str is equal to std::string \a std_str
*/
bool LORD_CORE_API	operator==(const std::string& std_str, const GUIString& str);

/*!
\brief
	Return true if GUIString \a str is equal to null-terminated utf8 data \a utf8_str
*/
bool LORD_CORE_API	operator==(const GUIString& str, const utf8* utf8_str);

/*!
\brief
	Return true if GUIString \a str is equal to null-terminated utf8 data \a utf8_str
*/
bool LORD_CORE_API	operator==(const utf8* utf8_str, const GUIString& str);

/*!
\brief
	Return true if GUIString \a str1 is not equal to GUIString \a str2
*/
bool LORD_CORE_API	operator!=(const GUIString& str1, const GUIString& str2);

/*!
\brief
	Return true if GUIString \a str is not equal to std::string \a std_str
*/
bool LORD_CORE_API	operator!=(const GUIString& str, const std::string& std_str);
...
/*!
\brief
	Return true if GUIString \a str is not equal to null-terminated utf8 data \a utf8_str
*/
bool LORD_CORE_API	operator!=(const GUIString& str, const utf8* utf8_str);
...
/*!
\brief
	Return true if GUIString \a str1 is lexicographically less than GUIString \a str2
*/
bool LORD_CORE_API	operator<(const GUIString& str1, const GUIString& str2);

/*!
\brief
	Return true if GUIString \a str is lexicographically less than std::string \a std_str
*/
bool LORD_CORE_API	operator<(const GUIString& str, const std::string& std_str);
...
/*!
\brief
	Return true if GUIString \a str is lexicographically less than null-terminated utf8 data \a utf8_str
*/
bool LORD_CORE_API	operator<(const GUIString& str, const utf8* utf8_str);
...
/*!
\brief
	Return true if GUIString \a str1 is lexicographically greater than GUIString \a str2
*/
bool LORD_CORE_API	operator>(const GUIString& str1, const GUIString& str2);

/*!
\brief
	Return true if GUIString \a str is lexicographically greater than std::string \a std_str
*/
bool LORD_CORE_API	operator>(const GUIString& str, const std::string& std_str);
...
/*!
\brief
	Return true if GUIString \a str is lexicographically greater than null-terminated utf8 data \a utf8_str
*/
bool LORD_CORE_API	operator>(const GUIString& str, const utf8* utf8_str);
...
/*!
\brief
	Return true if GUIString \a str1 is lexicographically less than or equal to GUIString \a str2
*/
bool LORD_CORE_API	operator<=(const GUIString& str1, const GUIString& str2);

/*!
\brief
	Return true if GUIString \a str is lexicographically less than or equal to std::string \a std_str
*/
bool LORD_CORE_API	operator<=(const GUIString& str, const std::string& std_str);
...
/*!
\brief
	Return true if GUIString \a str is lexicographically less than or equal to null-terminated utf8 data \a utf8_str
*/
bool LORD_CORE_API	operator<=(const GUIString& str, const utf8* utf8_str);
...
/*!
\brief
	Return true if GUIString \a str1 is lexicographically greater than or equal to GUIString \a str2
*/
bool LORD_CORE_API	operator>=(const GUIString& str1, const GUIString& str2);

/*!
\brief
	Return true if GUIString \a str is lexicographically greater than or equal to std::string \a std_str
*/
bool LORD_CORE_API	operator>=(const GUIString& str, const std::string& std_str);
...
/*!
\brief
	Return true if GUIString \a str is lexicographically greater than or equal to null-terminated utf8 data \a utf8_str
*/
bool LORD_CORE_API	operator>=(const GUIString& str, const utf8* utf8_str);
...
/*!
\brief
	Return true if GUIString \a str is equal to c-string \a c_str
*/
bool LORD_CORE_API	operator==(const GUIString& str, const char* c_str);

/*!
\brief
	Return true if c-string \a c_str is equal to GUIString \a str
*/
bool LORD_CORE_API	operator==(const char* c_str, const GUIString& str);

/*!
\brief
	Return true if GUIString \a str is not equal to c-string \a c_str
*/
bool LORD_CORE_API	operator!=(const GUIString& str, const char* c_str);

/*!
\brief
	Return true if c-string \a c_str is not equal to GUIString \a str
*/
bool LORD_CORE_API	operator!=(const char* c_str, const GUIString& str);

/*!
\brief
	Return true if GUIString \a str is lexicographically less than c-string \a c_str
*/
bool LORD_CORE_API	operator<(const GUIString& str, const char* c_str);

/*!
\brief
	Return true if c-string \a c_str is lexicographically less than GUIString \a str
*/
bool LORD_CORE_API	operator<(const char* c_str, const GUIString& str);

/*!
\brief
Return true if GUIString \a str is lexicographically greater than c-string \a c_str
*/
bool LORD_CORE_API	operator>(const GUIString& str, const char* c_str);

/*!
\brief
Return true if c-string \a c_str is lexicographically greater than GUIString \a str
*/
bool LORD_CORE_API	operator>(const char* c_str, const GUIString& str);

/*!
\brief
	Return true if GUIString \a str is lexicographically less than or equal to c-string \a c_str
*/
bool LORD_CORE_API	operator<=(const GUIString& str, const char* c_str);

/*!
\brief
	Return true if c-string \a c_str is lexicographically less than or equal to GUIString \a str
*/
bool LORD_CORE_API	operator<=(const char* c_str, const GUIString& str);

/*!
\brief
	Return true if GUIString \a str is lexicographically greater than or equal to c-string \a c_str
*/
bool LORD_CORE_API	operator>=(const GUIString& str, const char* c_str);

/*!
\brief
	Return true if c-string \a c_str is lexicographically greater than or equal to GUIString \a str
*/
bool LORD_CORE_API	operator>=(const char* c_str, const GUIString& str);
...
/*!
\brief
	Return GUIString object that is the concatenation of the given inputs

\param str1
	GUIString object describing first part of the new string

\param str2
	GUIString object describing the second part of the new string

\return
	A GUIString object that is the concatenation of \a str1 and \a str2

\exception std::length_error	Thrown if the resulting GUIString would be too large.
*/
GUIString LORD_CORE_API	operator+(const GUIString& str1, const GUIString& str2);
...
\return
	A GUIString object that is the concatenation of \a str and \a std_str

\exception std::length_error	Thrown if the resulting GUIString would be too large.
*/
GUIString LORD_CORE_API	operator+(const GUIString& str, const std::string& std_str);
...
\return
	A GUIString object that is the concatenation of \a std_str and \a str

\exception std::length_error	Thrown if the resulting GUIString would be too large.
*/
GUIString LORD_CORE_API	operator+(const std::string& std_str, const GUIString& str);
...
\return
	A GUIString object that is the concatenation of \a str and \a utf8_str

\exception std::length_error	Thrown if the resulting GUIString would be too large.
*/
GUIString LORD_CORE_API	operator+(const GUIString& str, const utf8* utf8_str);
...
\exception std::length_error	Thrown if the resulting GUIString would be too large.
*/
GUIString LORD_CORE_API	operator+(const utf8* utf8_str, const GUIString& str);
...
\param str
	GUIString object describing the first part of the new string

\param code_point
	utf16 code point describing the second part of the new string

\return
	A GUIString object that is the concatenation of \a str and \a code_point

\exception std::length_error	Thrown if the resulting GUIString would be too large.
*/
GUIString LORD_CORE_API	operator+(const GUIString& str, utf16 code_point);
...
\param code_point
	utf16 code point describing the first part of the new string

\param str
	GUIString object describing the second part of the new string

\return
	A GUIString object that is the concatenation of \a code_point and \a str

\exception std::length_error	Thrown if the resulting GUIString would be too large.
*/
GUIString LORD_CORE_API	operator+(utf16 code_point, const GUIString& str);
...
\return
	A GUIString object that is the concatenation of \a str and \a c_str

\exception std::length_error	Thrown if the resulting GUIString would be too large.
*/
GUIString LORD_CORE_API	operator+(const GUIString& str, const char* c_str);
...
\return
	A GUIString object that is the concatenation of \a c_str and \a str

\exception std::length_error	Thrown if the resulting GUIString would be too large.
*/
GUIString LORD_CORE_API	operator+(const char* c_str, const GUIString& str);


//////////////////////////////////////////////////////////////////////////
// Output (stream) functions
//////////////////////////////////////////////////////////////////////////
LORD_CORE_API std::ostream& operator<<(std::ostream& s, const GUIString& str);
...
/*!
\brief
	Swap the contents for two GUIString objects

\param str1
	GUIString object who's contents are to be swapped with \a str2

\param str2
	GUIString object who's contents are to be swapped with \a str1

\return
	Nothing
*/
void LORD_CORE_API swap(GUIString& str1, GUIString& str2);
