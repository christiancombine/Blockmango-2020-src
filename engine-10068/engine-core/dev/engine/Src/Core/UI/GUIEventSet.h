...
public:
	...
	*/
	...
	\param name
		GUIString object containing the name to give the new Event.  The name must be unique for the EventSet.
	...
	*/
	void	addEvent(const GUIString& name);
	...
	\param name
		GUIString object containing the name of the Event to remove.  If no such Event exists, nothing happens.

	\return
		Nothing.
	*/
	void	removeEvent(const GUIString& name);
	...
	*/
	bool	isEventPresent(const GUIString& name);
    ...
    */
    virtual GUIEvent::Connection subscribeEvent(const GUIString& name, GUIEvent::Subscriber subscriber);
    ...
    \return
        ...
        connection and to disconnect (unsubscribe) from the Event.
    */
    virtual GUIEvent::Connection subscribeEvent(const GUIString& name, GUIEvent::Group group, GUIEvent::Subscriber subscriber);
    ...
    \param subscriber_name
        GUIString object containing the name of the script funtion that is to be
    ...
    \return
        ...
        connection and to disconnect (unsubscribe) from the Event.
    */
    virtual GUIEvent::Connection subscribeScriptedEvent(const GUIString& name, const GUIString& subscriber_name);
    ...
    \return
        ...
        connection and to disconnect (unsubscribe) from the Event.
    */
    virtual GUIEvent::Connection subscribeScriptedEvent(const GUIString& name, GUIEvent::Group group, const GUIString& subscriber_name);
    ...
    \param name
        GUIString object holding the name of the Event that is to be fired
        (triggered)
    ...
    \param eventNamespace
        GUIString object describing the global event namespace prefix for this
        event.
...
protected:
    ...
    \param name
        GUIString object holding the name of the Event to return.
    ...
    */
    GUIEvent* getEventObject(const GUIString& name, bool autoAdd = false);

    /*!
    \brief
        Implementation event firing member
    */
    void fireEvent_impl(const GUIString& name, EventArgs& args);
	...
	typedef std::map<GUIString, GUIEvent*, GUIString::FastLessCompare>	EventMap;
	EventMap	d_events;
