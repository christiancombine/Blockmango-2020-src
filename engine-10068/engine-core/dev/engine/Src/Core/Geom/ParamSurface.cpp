...
namespace LORD
{
	...
	uint ParamSurface::getVertexCount() const
	{
		return (uint)m_vDivision.x * (uint)m_vDivision.y;
	}

	uint ParamSurface::getLineIndexCount() const
	{
		return 4 * (uint)m_vSlice.x * (uint)m_vSlice.y;
	}

	uint ParamSurface::getTriangleIndexCount() const
	{
		return 6 * (uint)m_vSlice.x * (uint)m_vSlice.y;
	}

	void ParamSurface::generateVertices(Buffer& buff, bool bNormal, bool bTexCoord) const
	{
		uint vertStride = sizeof(Vector3);
		if (bNormal)
		...
		uint offset = 0;
		for(uint j = 0; j < m_vDivision.y; ++j)
		{
			for(uint i = 0; i < m_vDivision.x; ++i)
			{
				// compute position
	...
	void ParamSurface::generateLineIndices(Buffer& buff, uint& stride) const
	{
		stride = (getLineIndexCount() > Math::MAX_WORD)? sizeof(Dword) : sizeof(Word);
		buff.allocate(getLineIndexCount()*stride);
		uint offset = 0;
		for(uint j = 0, nVert = 0; j < m_vSlice.y; ++j)
		{
			for(uint i = 0; i < m_vSlice.x; ++i)
			{
				uint next = (i + 1) % (uint)m_vDivision.x;
				...
				if(stride == sizeof(Word))
				{
					...
					wIndices[2] = nVert + i;
					wIndices[3] = nVert + (uint)m_vDivision.x;
				...
				else
				{
					...
					dwIndices[2] = nVert + i;
					dwIndices[3] = nVert + (uint)m_vDivision.x;
					memcpy(buff.getData() + offset, dwIndices, 4 * stride);
					offset += 4 * stride;
				}
			}

			nVert += (uint)m_vDivision.x;
		}
	}

	void ParamSurface::generateTriangleIndices(Buffer& buff, uint& stride) const
	{
		...
		for(uint j = 0, nVert = 0; j < m_vSlice.y; ++j)
		{
			for(uint i = 0; i < m_vSlice.x; ++i)
			{
				...
				if(stride == sizeof(Word))
				{
					...
					wIndices[0] = nVert + i;
					wIndices[1] = nVert + next;
					wIndices[2] = nVert + i + (uint)m_vDivision.x;
					wIndices[3] = nVert + next;
					wIndices[4] = nVert + next + (uint)m_vDivision.x;
					wIndices[5] = nVert + i + (uint)m_vDivision.x;
				...
				else
				{
					...
					dwIndices[0] = nVert + i;
					dwIndices[1] = nVert + next;
					dwIndices[2] = nVert + i + (uint)m_vDivision.x;
					dwIndices[3] = nVert + next;
					dwIndices[4] = nVert + next + (uint)m_vDivision.x;
					dwIndices[5] = nVert + i + (uint)m_vDivision.x;
