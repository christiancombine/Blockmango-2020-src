...
bool dtNavMeshQuery::rayDetect( const LORD::Vector3& startPos, const LORD::Vector3& dir, float& oLength, const dtQueryFilter* filter, LORD::Vector3& oHitPosition) const
{
	dtAssert(m_nav);

	LORD::Vector3 extends= LORD::Vector3(abs(dir.x), abs(dir.y), abs(dir.z)) * oLength * 0.5f;
	LORD::Vector3 center = startPos + dir * oLength * 0.5f;

	int		  polyCount;
	dtPolyRef polyRefs[512];
	if( DT_SUCCESS==queryPolygons( &center[0], &extends[0], filter, polyRefs, &polyCount, 512))
	{
		LORD::Ray      ray( startPos, dir);
		LORD::Triangle triangle;
		float    resultLen = 1e30f;
		for ( int i=0; i<polyCount; i++)
		{
			const dtMeshTile* tile = 0;
			const dtPoly*     poly = 0;
			if (dtStatusFailed(m_nav->getTileAndPolyByRef( polyRefs[i], &tile, &poly)))
				return false;
			if (!tile)
				return false;
			...
			for (int j = 0; j < pd->triCount; ++j)
			{
				...
				for (int k = 0; k < 3; ++k)
				{
					float* tv = NULL;
					if (t[k] < poly->vertCount)
						v[k] = (LORD::Vector3&)tile->verts[poly->verts[t[k]]*3];
					else
						v[k] = (LORD::Vector3&)tile->detailVerts[(pd->vertBase+(t[k]-poly->vertCount))*3];
				}
