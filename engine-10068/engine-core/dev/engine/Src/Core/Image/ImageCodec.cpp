...
#if (LORD_ENDIAN == LORD_ENDIAN_BIG)
...
namespace LORD
{
	...
	bool ImageCodec::doEncode(ImageFormat imgFmt, const Buffer& inBuff, Buffer& outBuff, const Image::ImageInfo& imgInfo) const
	{
		...
		{
			LordLogError("Don't support the pixel format [%s].", PixelUtil::GetPixelFormatName(requiredFormat).c_str());
			return false;
		}
		...
		// Check BPP
		uint bpp = PixelUtil::GetPixelBits(requiredFormat);
		if(!FreeImage_FIFSupportsExportBPP(fiImgFmt, (int)bpp))
		{
			...
			{
				LordLogError(	"Don't support the image type ( ImageFormat [%s], BPP [%d], PixelFormat [%s] ).", 
		...
		if(requiredFormat != imgInfo.pixFmt)
		{
			...
			// Allocate memory
			convBox.pData = LORD_ALLOC_T(Byte, convBox.getConsecutiveSize());
		...
		if(!pBitmap)
		{
			if (conversionRequired)
				LORD_FREE(convBox.pData);

			LordLogError("FreeImage_AllocateT failed - possibly out of memory.");
			LordAssertX(false, "FreeImage_AllocateT failed - possibly out of memory.");
		...
		uint dstPitch = FreeImage_GetPitch(pBitmap);
		uint srcPitch = imgInfo.width * PixelUtil::GetPixelSize(requiredFormat);

		// Copy data, invert scanlines and respect FreeImage pitch
		Byte* pDst = FreeImage_GetBits(pBitmap);
		uint dibSize = FreeImage_GetDIBSize(pBitmap);
		for(uint y = 0; y < imgInfo.height; ++y)
		{
		...
		//uint buffSize = srcPitch * imgInfo.height;
	...
	DataStream* ImageCodec::doDecode(ImageFormat imgFmt, DataStream* inStream, Image::ImageInfo& imgInfo)
	{
		...
		if (!fiBitmap)
		{
			LordLogError("Error decoding image format [%s].", Image::GetImageFormatName(imgFmt).c_str());
			return NULL;
		}
		...
		default:
			{
				LordLogError("Unknown or unsupported image format [%s].", Image::GetImageFormatName(imgFmt).c_str());
				return NULL;
			}
			break;
		case FIT_BITMAP:
		...
		uint dstPitch = tempImgInfo.width * PixelUtil::GetPixelSize(tempImgInfo.pixFmt);
		tempImgInfo.size = dstPitch * tempImgInfo.height;

		imgInfo = tempImgInfo;

		// Bind output buffer
		MemoryDataStream* pResult = LordNew MemoryDataStream(tempImgInfo.size, false);
