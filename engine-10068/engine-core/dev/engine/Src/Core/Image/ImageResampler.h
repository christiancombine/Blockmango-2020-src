...
#ifndef __LORD_IMAGERESAMPLER_H__
#define __LORD_IMAGERESAMPLER_H__
...
namespace LORD
{
	...
	template<uint elemsize> struct NearestResampler
	{
		static void Scale(const PixelBox& src, const PixelBox& dst)
		{
			...
			for(uint z = dst.front; z < dst.back; z++, sz_48 += stepz)
			{
				uint srczoff = (uint)(sz_48 >> 48) * src.slicePitch;

				ui64 sy_48 = (stepy >> 1) - 1;
				for(uint y = dst.top; y < dst.bottom; y++, sy_48 += stepy)
				{
					uint srcyoff = (uint)(sy_48 >> 48) * src.rowPitch;

					ui64 sx_48 = (stepx >> 1) - 1;
					for(size_t x = dst.left; x < dst.right; x++, sx_48 += stepx)
					{
						Byte* psrc = srcdata + elemsize*((uint)(sx_48 >> 48) + srcyoff + srczoff);
	...
	struct LinearResampler
	{
		static void Scale(const PixelBox& src, const PixelBox& dst)
		{
			uint srcelemsize = PixelUtil::GetPixelSize(src.pixFmt);
			uint dstelemsize = PixelUtil::GetPixelSize(dst.pixFmt);
			...
			for(uint z = dst.front; z < dst.back; z++, sz_48+=stepz)
			{
				temp = static_cast<uint>(sz_48 >> 32);
				temp = (temp > 0x8000)? temp - 0x8000 : 0;
				uint sz1 = temp >> 16;				 // src z, sample #1
				uint sz2 = Math::Min(sz1+1, src.getDepth()-1);// src z, sample #2
				float szf = (temp & 0xFFFF) / 65536.f; // weight of sample #2

				ui64 sy_48 = (stepy >> 1) - 1;
				for(uint y = dst.top; y < dst.bottom; y++, sy_48+=stepy)
				{
					temp = static_cast<uint>(sy_48 >> 32);
					temp = (temp > 0x8000)? temp - 0x8000 : 0;
					uint sy1 = temp >> 16;					// src y #1
					uint sy2 = Math::Min(sy1+1, src.getHeight()-1);// src y #2
					float syf = (temp & 0xFFFF) / 65536.f; // weight of #2

					ui64 sx_48 = (stepx >> 1) - 1;
					for(uint x = dst.left; x < dst.right; x++, sx_48 += stepx)
					{
						temp = static_cast<uint>(sx_48 >> 32);
						temp = (temp > 0x8000)? temp - 0x8000 : 0;
						uint sx1 = temp >> 16;					// src x #1
						uint sx2 = Math::Min(sx1+1,src.getWidth()-1);// src x #2
...
#undef UNPACK
	...
	struct LinearResamplerFloat32
	{
		static void Scale(const PixelBox& src, const PixelBox& dst)
		{
			uint srcchannels = PixelUtil::GetPixelSize(src.pixFmt) / sizeof(float);
			uint dstchannels = PixelUtil::GetPixelSize(dst.pixFmt) / sizeof(float);
...
#define ACCUM3(x, y, z, factor) { float f = factor;	uint off = (x+y*src.rowPitch+z*src.slicePitch)*srcchannels; accum[0]+=srcdata[off+0]*f; accum[1]+=srcdata[off+1]*f;	accum[2]+=srcdata[off+2]*f; }

#define ACCUM4(x, y, z, factor) { float f = factor;	uint off = (x+y*src.rowPitch+z*src.slicePitch)*srcchannels; accum[0]+=srcdata[off+0]*f; accum[1]+=srcdata[off+1]*f; accum[2]+=srcdata[off+2]*f; accum[3]+=srcdata[off+3]*f; }
...
#undef ACCUM4
	...
	template<uint channels> struct LinearResamplerByte
	{
		static void Scale(const PixelBox &src, const PixelBox &dst)
		{
			// assert(src.format == dst.format);
			...
			for(uint y = dst.top; y < dst.bottom; ++y, sy_48 += stepy)
			{
				temp = static_cast<uint>(sy_48 >> 36);
				temp = (temp > 0x800)? temp - 0x800: 0;
				uint syf = temp & 0xFFF;
				uint sy1 = temp >> 12;
				uint sy2 = Math::Min(sy1+1, src.bottom-src.top-1);
				uint syoff1 = sy1 * src.rowPitch;
				uint syoff2 = sy2 * src.rowPitch;

				ui64 sx_48 = (stepx >> 1) - 1;
				for(uint x = dst.left; x < dst.right; ++x, sx_48 += stepx)
				{
					temp = static_cast<uint>(sx_48 >> 36);
					temp = (temp > 0x800)? temp - 0x800 : 0;
					unsigned int sxf = temp & 0xFFF;
					uint sx1 = temp >> 12;
					uint sx2 = Math::Min(sx1+1, src.right-src.left-1);
