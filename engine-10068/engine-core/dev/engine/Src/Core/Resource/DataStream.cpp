...
namespace LORD
{
	...
	//-----------------------------------------------------------------------
	FileStreamDataStream::FileStreamDataStream(const String& name,
		std::istream* s, size_t size, bool freeOnClose, bool useMemPool)
		: DataStream(name), mpInStream(s), mpFStreamRO(0), mpFStream(0), mFreeOnClose(freeOnClose), mUseMemPool(useMemPool)
	{
		// Size is passed in
		mSize = size;
		determineAccess();
