...
public class GuiIngame extends Gui
{
    ...
    public void renderGameOverlay(float par1, boolean par2, int par3, int par4)
    {
        ...
        if (this.mc.thePlayer.func_110317_t())
        {
            ...
            var34 = this.mc.thePlayer.func_110319_bJ();
            var35 = 182;
        ...
        if (this.mc.gameSettings.showDebugInfo)
        {
            ...
            /** add by zhouyou. draw the lit value*/
            String rayTrace = "RayTrace ";
            String neighbor = "  ";
            if (mc.objectMouseOver != null &&  mc.objectMouseOver.typeOfHit == EnumMovingObjectType.TILE)
            {
                ...
                int skylight = mc.theWorld.getSkyBlockTypeBrightness(EnumSkyBlock.Sky, x, y, z);
                int blocklight = mc.theWorld.getSkyBlockTypeBrightness(EnumSkyBlock.Block, x, y, z);
                rayTrace += "[" + x + "," + y + "," + z + "] id:" + " lit[" + skylight + "," + blocklight + "]";
                for (int i = 0; i < 6; ++i)
                {
                    neighbor += "  ";
                    int nx = x + Facing.offsetsXForSide[i];
                    int ny = y + Facing.offsetsYForSide[i];
                    int nz = z + Facing.offsetsZForSide[i];
                    blockID = mc.theWorld.getBlockId(nx, ny, nz);
                    skylight = mc.theWorld.getSkyBlockTypeBrightness(EnumSkyBlock.Sky, nx, ny, nz);
                    blocklight = mc.theWorld.getSkyBlockTypeBrightness(EnumSkyBlock.Block, nx, ny, nz);
                    neighbor += Facing.facings[i].charAt(0) + " id:" + blockID + " lit[" + skylight + "," + blocklight + "]";
                }
            }
            this.drawString(var8, rayTrace, 2, 112, 14737632);
            this.drawString(var8, neighbor, 2, 120, 14737632);
