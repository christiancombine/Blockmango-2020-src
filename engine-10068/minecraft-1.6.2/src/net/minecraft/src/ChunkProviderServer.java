...
public class ChunkProviderServer implements IChunkProvider
{
    ...
    // added by zhouyou
    public void saveAllChunks(String folderName)
    {
        new File(folderName).mkdirs();

        for (int i = 0; i < this.loadedChunks.size(); ++i)
        {
            Chunk ck = (Chunk)this.loadedChunks.get(i);

            if (ck == null)
                continue;

            String FileName = String.format("%s\\chunk_%03d_%03d.java", folderName, ck.xPosition, ck.zPosition);
            try
            {
                DataOutputStream out=new DataOutputStream(new FileOutputStream(FileName));
                ExtendedBlockStorage[] sections = ck.getBlockStorageArray();

                for(int j=0; j<sections.length; ++j)
                {
                    if(sections[j] == null)
                        continue;
                    byte[] lsbArray = sections[j].getBlockLSBArray();
                    if(lsbArray == null)
                        continue;
                    String txt = "";
                    ...
                    for (int y = 0; y < 16; ++y)
                    {
                        for (int z = 0; z < 16; ++z)
                        {
                            ...
                            for (int x = 0; x < 16; ++x)
                            {
                                int bkid = lsbArray[y << 8 | z << 4 | x] & 255;
                                txt += bkid;
                                if (x != 15)
                                    txt += ", ";
                            }
